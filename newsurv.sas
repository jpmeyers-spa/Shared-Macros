...
|                             TOTALMV, EVENTMV, EV_NMV, N_EVMV, HRMV, PVALMV, COVPVALMV, and CINDEXMV
...
%macro newsurv(
    ...
    tpvalwidth=0.7in,tpvalmvwidth=0.7in,ttimelistwidth=1.6in,ttimelistmvwidth=1.6in,
...
%macrolog(newsurv);
    ...
    %do z = 1 %to &nmodels;
        ...
        %if %sysevalf(%qupcase(%superq(method&z))=KM,boolean) %then 
            %let _displaylist=legend|hr|median|total|event|ev_n|n_ev|timelist|pval|cindex|tablecomments|totalmv|eventmv|ev_nmv|n_evmv|hrmv|pvalmv|cindexmv|covpval|covpvalmv|RMST|RMTL|RMST_PVAL;
        %else %if %sysevalf(%qupcase(%superq(method&z))^=CIF,boolean) %then 
            %let _displaylist=legend|hr|median|total|event|ev_n|n_ev|timelist|pval|cindex|tablecomments;
        %else %if %sysevalf(%qupcase(%superq(method&z))=CIF,boolean) %then %let _displaylist=legend|hr|median|total|event|ev_n|n_ev|timelist|pval|tablecomments|totalmv|eventmv|ev_nmv|n_evmv|hrmv|pvalmv|covpval|covpvalmv;
        /**Check for missing values**/
    ...
    %if %sysevalf(%superq(tabledisplay)=,boolean)=0 %then %do _z2=1 %to %sysfunc(countw(%superq(tabledisplay),%str( )));
        %let _test=;
        %let _list=title|footnote|total|event|ev_n|n_ev|median|hr|timelist|pval|covpval|cindex|totalmv|eventmv|ev_nmv|n_evmv|hrmv|medianmv|timelistmv|pvalmv|covpvalmv|cindexmv|RMST|RMTL|RMST_PVAL;
    ...
    %do z = 1 %to &nmodels;/**Start of Analysis Section**/
        ...
        data _tempdsn&z;
            merge %if %sysevalf(%superq(class&z)=,boolean)=0 %then %do; 
                  ...
                  %end;;
            %if %sysevalf(%superq(class&z)=,boolean)=0 %then %do; 
                length _class_ $300. _class_order 8.;
        ...
        proc sql noprint;
            ...
            %if %sysevalf(%superq(class&z)=,boolean)=0 %then %do;     
                ...
                select count(distinct _class_) into :nclass_&z from _tempdsn&z
                    where _multi_=0; 
            %end;
            %else %let nclass_&z=1;
            /***Save class values into macro variables***/
            %do i = 1 %to %superq(nclass_&z);
                %local class_&z._&i nclass_&z._&i;
            %end;            
            ...
            %if %sysevalf(%superq(class&z)=,boolean)=0 %then %do; 
                ...
                update _tempdsn&z
                    set _class_order=case (_class_)
                        %do i=1 %to &&nclass_&z;
                            when "%superq(class_&z._&i)" then &i
                        %end;
                        else . end
                    ;
                %if %qupcase(&&method&z)=INVWTS %then %do;
                    /**Get counts for each Class Level**/            
                    select %do i = 1 %to %superq(nclass_&z);
                                %if &i>1 %then %do; , %end;
                                sum(ifn(_class_order=&i,1,0))
                            %end;
                            into %do i = 1 %to %superq(nclass_&z);
                                    %if &i>1 %then %do; , %end;
                                    :nclass_&z._&i separated by ' '
                                 %end;
                            from _tempdsn&z
                            where _multi_=1;
                %end;
                ...
                %local _classrefcheck _classreforder&z;
                %let _classrefcheck=;
                %if %sysevalf(%superq(classref&z)=,boolean)=0 %then %do;
                    select distinct 1,_class_order into :_classrefcheck,:_classreforder&z separated by ''
                        from _tempdsn&z
                        where strip(_class_)=strip("%superq(classref&z)") and _multi_=0;
                ...
                %end;  
                %else %do;
                    select distinct _class_,_class_order into :classref&z separated by '',:_classreforder&z separated by '' from _tempdsn&z where ^missing(_class_) 
        ...
        proc lifetest data=_tempdsn&z alpha=&&alpha&z alphaqt=&&alpha&z            
            ...
            %if %sysevalf(%qupcase(&&method&z)=CIF,boolean) %then %do;
                ...
                outcif=_surv (where=(_multi_=0 and (event>. or t1=0 or c1=1))
                    rename=(%if %sysevalf(%superq(class&z)=,boolean)=0 %then %do; stratum=stratumnum %end;
                        _time_=t1 cif=s1 censored=c1 cif_lcl=lcl1 cif_ucl=ucl1 variance=vcif)
                    keep=_multi_ cif _time_ censored event atrisk cif_lcl cif_ucl variance
                        %if %sysevalf(%superq(class&z)=,boolean)=0 %then %do; stratum %end;)
            %end;     
            ...
            ods output 
                ...
                %if %qupcase(&&method&z)^=CIF %then %do;
                    survivalplot=_surv (where=(event>. and _multi_=0)
                        ...
                        keep=_multi_ survival time censored event sdf_lcl sdf_ucl
                            %if %sysevalf(%superq(class&z)=,boolean)=0 %then %do; stratumnum %end;)/**Set up dataset with survival numbers**/
                    ...
                    %if %sysevalf(%superq(risklist&z)=,boolean)=0 %then %do;
                        survivalplot=_splot
                        (keep=_multi_ time tatrisk stratumnum atrisk event censored) /**Outputs dataset with patients-at-risk numbers**/
                    %end;
                %end;
        ...
        %if %qupcase(&&method&z)=CIF %then %do;
            ...
            /**Generate Median quartiles table**/
                create table _quart %if %sysevalf(%superq(class)^=,boolean) %then %do; (rename=(stratumnum=_class_order)) %end; as  
                    select _multi_,%if %sysevalf(%superq(class)^=,boolean) %then %do; stratumnum,%end; 50 as percent,
                    ...
                    where _multi_=0
                    group by _multi_
                    %if %sysevalf(%superq(class)^=,boolean) %then %do; ,stratumnum %end;;
            quit;
        %end;
        %if %sysevalf(%superq(risklist&z)=,boolean)=0 and %qupcase(&&method&z)^=CIF %then %do;
            data _splot;
                set _splot;
                by _multi_ stratumnum;
                if first.stratumnum then do;
                    _ncens=0;_nevents=0;
                end;
                if event>. then _nevents=_nevents+event;
                ...
                proc sql noprint;
                    ...
                    select max(n),min(n) into :rm_max separated by '',:rm_min separated by '' from
                        (select %do i = 1 %to %sysfunc(countw(%superq(strata&z),%str( ))); _strata_&i, %end; count(distinct _class_order) as n
                            from _tempdsn&z where _multi_=0 group by _multi_ %do i = 1 %to %sysfunc(countw(%superq(strata&z),%str( ))); ,_strata_&i %end;);
                quit;
        ...
        %if %qupcase(&&method&z)=INVWTS %then %do;
            ...
            data _adjwts_prep;
                ...
                do i = 1 to %superq(nclass_&z);
                    _set_=i;
                    if _class_order=i then _adjevent_=1;
                    else _adjevent_=0;
                    output;
                end;
                drop i;
            ...
            data _adjwts;
                merge %do i = 1 %to %superq(nclass_&z); _adjwts (where=(_set_=&i) rename=(_adjwts_=_adjwts_&i) %if &i>1 %then %do; keep=_set_ _order_ _adjwts_ %end;) %end;;
                by _order_;                
                array _class_n_ {%superq(nclass_&z)} (%do i = 1 %to %superq(nclass_&z); 
                                                        %if &i>1 %then %do; , %end; 
                                                        %superq(nclass_&z._&i) 
                                                    %end;);
                array _adjwts_ {%superq(nclass_&z)};
                                                    
                do i = 1 to %superq(nclass_&z);
                    if _class_order=i then _adjwt_=(_class_n_(i)/sum(of _class_n_(*)))/_adjwts_(i);
                end;   
                drop _set_ _order_ _adjevent_ _level_ i _class_n_: _adjwts_:;
            run;
        ...
        %if %qupcase(&&method&z)=DIRECT %then %do;
            ...
            data _covs;
                set %do j=1 %to %superq(nclass_&z);
                        _tempdsn&z (in=a&j where=(_multi_=1))
                    %end;;
                %do j=1 %to %superq(nclass_&z);       
                    %if &j > 1 %then %do; else %end; 
                    if a&j then _class_order=&j;
                %end;
            run; 
            ods graphics on;
            ...
            run;
            data _surv_adj;
                merge _surv_adj (in=a drop=c1) _surv_adj2 (keep=stratumnum t1 c1 where=(^missing(c1)));
                by stratumnum t1;
                if a or ^missing(c1);
            run;
            data _surv_adj;
            ...
            data _quartmv (keep=stratumnum percent estimate lowerlimit upperlimit rename=(stratumnum=_class_order))
                %if %sysevalf(%superq(timelist&z)^=,boolean) %then %do;
                    _timelistmv (keep= stratumnum timelist _time_ survival sdf_lcl sdf_ucl rename=(stratumnum=_class_order))
                %end;;
                set _surv_adj (rename=(t1=_time_ s1=_surv_ lcl1=_lcl_ ucl1=_ucl_)) end=last;
                by stratumnum _time_;
        ...
        %if %sysevalf(%superq(class&z)=,boolean)=0 %then %do;
            ...
            proc phreg data=_tempdsn&z alpha=&&alpha&z;
                ...
                class 
                    ...
                    _class_order %if %sysevalf(%superq(classref&z)=,boolean)=0 %then %do; (ref="%superq(_classreforder&z)") %end;
                        /**Class covariates**/
                        %if %sysevalf(%superq(classcov&z)=,boolean) =0 %then %do i = 1 %to %sysfunc(countw(%superq(classcov&z),%str( ))); 
                            _classcov_&i 
                        %end;;
        ...
        proc sql noprint;
            ...
            ***Check if output table has been determined;
            ...
            insert into _temptable 
                (modelnum,modeltype,classlevel,title,footnote,subind,subtitle,
                 %if %sysevalf(%superq(class&z)=,boolean) %then %do; 
                 ...
                 %else %do;
                    ...
                    %if %sysevalf(%superq(classcov&z)^=,boolean) or %sysevalf(%superq(contcov&z)^=,boolean) %then %do;   
                        ,pvalnmv,pvalmv,cindexmv_estimate,cindexmv_lcl,cindexmv_ucl,cindexmv
                    %end;
                 %end;
                    )
                select sum(&z,&_model) as modelnum,
                    ...
                    1 as subind, put(coalescec("%superq(classdesc&z)" %if %sysevalf(%superq(class&z)^=,boolean) %then %do; ,"%superq(label&z)" %end;,""),$100.) as subtitle,
                    %if %sysevalf(%superq(class&z)=,boolean) %then %do; 
                    ...
                    %end;                    
                    %else %do;
                        b.pval as pvaln,strip(put(b.pval,pvalue%sysevalf(6.&&pvaldigits&z)))||
                        ...
                        %if %qupcase(&&method&z)=KM and %sysfunc(exist(_rmstp))^=0 %then %do;
                            c.probchisq as rmst_pvaln,strip(put(c.probchisq,pvalue%sysevalf(6.&&pvaldigits&z))) as rmst_pval,
                        %end;
                        ...
                        %if %sysevalf(%superq(classcov&z)^=,boolean) or %sysevalf(%superq(contcov&z)^=,boolean) %then %do;                        
                            ,b2.pval as pvalnmv,strip(put(b2.pval,pvalue%sysevalf(6.&&pvaldigits&z)))||
                ...
                from 
                    %if %sysevalf(%superq(class&z)=,boolean) %then %do; 
                        (select * from _sum where _multi_=0) a left join 
                            (select * from _quart where _multi_=0) b on a._multi_=b._multi_
                            %if %qupcase(&&method&z)=KM %then %do;
                                left join (select * from _rmst where _multi_=0) c on a._multi_=c._multi_
                                left join (select * from _rmtl where _multi_=0) c2 on a._multi_=c2._multi_
                            %end;                            
                             %if %sysevalf(%superq(timelist&z)=,boolean)=0 %then %do;
                                left join (select * from _timelist where _multi_=0) d on a._multi_=d._multi_
                            %end;
                    %end;
                    %else %do;
                        (select distinct _multi_ from _sum where _multi_=0) a left join 
                            %if %qupcase(%superq(plotpval&z))=LR or %qupcase(%superq(plotpval&z))=SCORE %then %do;
                                (select *,prob&&plotpval&z..chisq as pval from _t3 where _multi_=0 and upcase(effect)=upcase(strip("_class_order"))) as b on a._multi_=b._multi_
                            %end; 
                            %else %if %qupcase(%superq(plotpval&z))=WALD %then %do;
                                (select *,probchisq as pval from _t3 where _multi_=0 and upcase(effect)=upcase(strip("_class_order"))) as b on a._multi_=b._multi_
                            %end;
                            %else %if %qupcase(%superq(plotpval&z))=LOGRANK_ONESIDED %then %do;
                                (select *,probchisq/2 as pval from _ltest where _multi_=0) as b on a._multi_=b._multi_
                            %end;
                            %else %do;
                                (select *,probchisq as pval from _ltest where _multi_=0) as b on a._multi_=b._multi_
                            %end;
                            %if %qupcase(&&method&z)=KM and %sysfunc(exist(_rmstp))^=0 %then %do;
                                left join (select * from _rmstp where _multi_=0) as c on a._multi_=c._multi_
                            %end;
                            left join (select * from _cindex where _bylevel_=0 and strata=.) d on a._multi_=d._bylevel_
                            %if %sysevalf(%superq(classcov&z)^=,boolean) or %sysevalf(%superq(contcov&z)^=,boolean) %then %do;
                                %if %qupcase(%superq(plotpvalmv&z))=LR or %qupcase(%superq(plotpvalmv&z))=SCORE %then %do;
                                    left join (select *,prob&&plotpvalmv&z..chisq as pval from _t3 where _multi_=1 and upcase(effect)=upcase(strip("_class_order"))) as b2 on a._multi_=(b2._multi_-1)
                                %end;
                                %else %if %qupcase(%superq(plotpvalmv&z))=WALD %then %do;
                                    left join (select *,probchisq as pval from _t3 where _multi_=1 and upcase(effect)=upcase(strip("_class_order"))) as b2 on a._multi_=(b2._multi_-1)
                                %end;
                                %else %do;
                                    left join (select *,probchisq as pval ,0 as _multi_ from _ltestmv) as b2 on a._multi_=b2._multi_
                                %end;
                                left join (select * from _cindex where _bylevel_=1 and strata=.) d2 on a._multi_=(d2._bylevel_-1)
                            %end;
                    %end;;
                    ...
                    insert into _temptable 
                        ...
                        select sum(&z,&_model) as modelnum,
                            ...
                            e.probchisq as covpvaln, put(ifc(^missing(e.parameter),
                                strip(put(e.probchisq,pvalue%sysevalf(6.&&pvaldigits&z)))||'^{super +}',"%superq(refptext&z)"),$50.) as covpval
                            %if %sysevalf(%superq(classcov&z)^=,boolean) or %sysevalf(%superq(contcov&z)^=,boolean) %then %do;
                               ,a2.total as totalmv, a2.failed as eventmv, 
                                ...
                                e2.probchisq as covpvalnmv, put(ifc(^missing(e2.parameter),strip(put(e2.probchisq,pvalue%sysevalf(6.&&pvaldigits&z)))||'^{super +}',"%superq(refptext&z)"),$50.) as covpvalmv
                            %end;
                        from 
                            (select * from _sum where _multi_=0 and missing(control_var)) a left join 
                                (select * from _quart where _multi_=0) b on a._multi_=b._multi_ and a._class_order=b._class_order
                                %if %qupcase(&&method&z)=KM %then %do;
                                    left join (select * from _rmst where _multi_=0) c on a._multi_=c._multi_ and a._class_order=c._class_order
                                    left join (select * from _rmtl where _multi_=0) c2 on a._multi_=c2._multi_ and a._class_order=c2._class_order
                                %end;                
                                %if %sysevalf(%superq(timelist&z)^=,boolean) %then %do;
                                    left join (select * from _timelist where _multi_=0) d on a._multi_=d._multi_ and a._class_order=d._class_order
                                %end;
                                left join (select * from _parm where _multi_=0 and upcase(strip(parameter))=upcase(strip("_class_order"))) e on a._multi_=e._multi_ and strip(put(a._class_order,12.))=e.classval0
                                %if %sysevalf(%superq(classcov&z)^=,boolean) or %sysevalf(%superq(contcov&z)^=,boolean) %then %do;
                                    left join (select * from _sum where _multi_=1 and missing(control_var)) a2 on a._class_order=a2._class_order
                                    %if %qupcase(&&method&z)=INVWTS %then %do;
                                        left join (select * from _quartmv) b2 on  a._class_order=b2._class_order                    
                                        %if %sysevalf(%superq(timelist&z)^=,boolean) %then %do;
                                            left join (select * from _timelistmv) d2 on a._class_order=d2._class_order and d.timelist=d2.timelist
                                        %end;
                                    %end;
                                    %else %if %qupcase(&&method&z)=DIRECT %then %do;
                                        ...
                                        %end;
                                    %end;
                                    left join (select * from _parm where _multi_=1 and upcase(strip(parameter))=upcase(strip("_class_order"))) e2 on strip(put(a._class_order,12.))=e2.classval0
                                %end;
                            ;
                %end;       
        quit;
        ...
        data _plot_&z;
            ...
            merge
                ...
                %do j = 0 %to %sysevalf((%qupcase(&&method&z)=INVWTS or %qupcase(&&method&z)=DIRECT));
                    ...
                    _surv&sfx. (
                        rename=(
                            %if %sysevalf(%superq(class&z)=,boolean)=0 %then %do; stratumnum=cln&sfx._&z %end; /**Class Variable order**/
            ...
            %do j = 0 %to %sysevalf((%qupcase(&&method&z)=INVWTS or %qupcase(&&method&z)=DIRECT));
                ...
                %if %sysevalf(%superq(class&z)=,boolean)=0 %then %do i=1 %to &&nclass_&z;
        ...
        %if %sysevalf(%superq(risklist&z)=,boolean)=0 %then %do;
            ...
            data _riskplot;
                    ...
                    %do j = 0 %to %sysevalf((%qupcase(&&method&z)=INVWTS or %qupcase(&&method&z)=DIRECT));
                        ...
                        %do i = 1 %to %superq(nclass_&z);
                            ...
                            _splot (rename=(
                                ...
                                where=(_multi_=&j and stratumnum=&i2))
                        %end;
                    %end;
                    ;
                ...
                label
                    ...
                    %end;
                    ;                          
                drop stratumnum tatrisk _multi_;   
            run;
        ...
        %if %sysevalf(%superq(display&z)=,boolean)=0 %then %do i = 1 %to %sysfunc(countw(%superq(display&z),%str( )));
            %if %qupcase(%scan(%superq(display&z),&i,%str( )))=PVAL or %qupcase(%scan(%superq(display&z),&i,%str( )))=PVALMV %then %let _pvalcol_&z=1;
            %end;
        ...
        %if %index(%qupcase(%superq(display&z)),LEGEND) > 0 %then %do;
            ...
            %if &&_pvalcol_&z=1 or %index(%qupcase(%superq(display&z)),CINDEX) > 0
        ...
        proc sql noprint;
            ...
            %if %sysevalf(%superq(class&z)^=,boolean) %then %do;
                ...
                /**RMST P-values**/
                %local rmst_pval&z;                
                select scan(rmst_pval,1,'^') into :rmst_pval&z
                    from _no_tl
        ...
        proc datasets nodetails nolist;
           %if &debug=0 %then %do;
               delete _temptable _ltest _parm _parmmv _quart _sum _summv _surv _t3 _t3mv _reflines _reflines_t _reflines_y 
    ...
    %do z = 1 %to &nmodels;
        ...
        %if %qupcase(&&method&z)=KM or %qupcase(&&method&z)=CIF %then %let _rows&z=%sysevalf(1+%sysfunc(countw(%superq(pardisplay&z),%str( )))
            *%superq(nclass_&z)*(1+%sysevalf(%qupcase(%superq(risklabellocation&z))=ABOVE,boolean)));
        %else %let _rows&z=%sysevalf(1+%sysfunc(countw(%superq(pardisplay&z),%str( )))
            *(1+&&plot_unadjust&z)*%superq(nclass_&z)*(1+%sysevalf(%qupcase(%superq(risklabellocation&z))=ABOVE,boolean)));
                            ...
                            layout lattice / columns=1 columndatarange=union opaque=false 
                                ...
                                layout overlay /  
                                    ...
                                    %if %superq(plotci&z) = 1 or 
                                        ...
                                        %if %qupcase(&&method&z)=INVWTS or %qupcase(&&method&z)=DIRECT %then %do;
                                            bandplot x=t_adj_&z limitlower=eval(%superq(xmult_&z) *lcl_adj_&z) 
                                                limitupper=eval(%superq(xmult_&z) *ucl_adj_&z) / type=step group=_ua_ci_&z
                                                display=(%if %sysevalf(%superq(plotcifill&z)=1,boolean) %then %do;
                                                            fill
                                                        %end;
                                                        %else %do;
                                    ...
                                    %if %superq(_ndisplay_class_&z) gt 0 or %superq(_ndisplay_model_&z)>0 %then %do; 
                                        ...
                                        layout gridded / 
                                            rows=%sysevalf(%sysevalf(%superq(_ndisplay_class_&z)>0,boolean) + %sysevalf(%superq(_ndisplay_model_&z)>0,boolean)) columns=1 border=false
                                            ...
                                            %if %sysevalf(%superq(_ndisplay_class_&z)>0,boolean) %then %do;
                                                layout gridded / 
                                                    %if %qupcase(&&method&z)=KM or %qupcase(&&method&z)=CIF %then %do;
                                                        rows=%sysevalf(%superq(nclass_&z)+1) /**One row per class level**/                                                
                                                    %end;
                                                    %else %do;
                                                        rows=%sysevalf((1+&&plot_unadjust&z)*%superq(nclass_&z)+1) /**One row per class level**/ 
                                                    %end;
                                                                    ...
                                                                    %do k = 1 %to %superq(_ndisplay_mstats_&z);
                                                                        %if %superq(_display_mstats&k._&z)=PVAL %then %do;
                                                                            ...
                                                                            entry halign=left
                                                                                %if %sysevalf(%superq(pval&sfx.header&z)=,boolean)=0 %then %do;
                                                                                    "%unquote(&&pval&sfx.header&z) %sysfunc(strip(&&pval&sfx.&z))"
                                                                                %end;
                                                                                %else %if %sysevalf(%superq(strata&z)^=,boolean) %then %do;
                                                                                    "Stratified %sysfunc(propcase(%superq(plotpval&sfx.&z))) P-value: %sysfunc(strip(%superq(pval&sfx.&z)))"
                                                                                %end;
                                                                                %else %do;
                                                                                    "&sfx2.%sysfunc(propcase(%superq(plotpval&sfx.&z))) P-value: %sysfunc(strip(%superq(pval&sfx.&z)))"
                                                                        ...
                                                                        %if %superq(_display_mstats&k._&z)=RMST_PVAL %then %do;
                                                                            ...
                                                                            %if %sysevalf(&&rmst_pvalheader&z=,boolean)=0 %then %do;
                                                                                "&&rmst_pvalheader&z %sysfunc(strip(%superq(rmst_pval&z)))"
                                                                            %end;
                                                                            %else %if %sysevalf(%superq(strata&z)^=,boolean) %then %do;
                                                                                "Stratified RMST Chi-square P-value: %sysfunc(strip(%superq(rmst_pval&z)))"
                                                                            %end;
                                                                            %else %do;
                                                                                "RMST Chi-square P-value: %sysfunc(strip(%superq(rmst_pval&z)))"
                                                                            %end; / valign=top textattrs=(color=&fontcolor size=%superq(ptabsize&z) family="%superq(ptabfamily&z)");
                                                                        %end;
                                                                        %if %superq(_display_mstats&k._&z)=CINDEX %then %do;
                                                                            ...
                                                                            %end;
                                                                            %else %do;
                                                                                "C-index (%sysevalf(100-100*&&alpha&z)% CI): %sysfunc(strip(%superq(cindex&z)))"
                                                                            %end; / valign=top textattrs=(color=&fontcolor size=%superq(ptabsize&z) family="%superq(ptabfamily&z)");
                                                                        %end;
                                                                        %if %superq(_display_mstats&k._&z)=CINDEXMV %then %do;
                                                                            ...
                                                                            %end;
                                                                            %else %do;
                                                                                "Multivariate C-index (%sysevalf(100-100*&&alpha&z)% CI): %sysfunc(strip(%superq(cindexmv&z)))"
                                                                            %end; / valign=top textattrs=(color=&fontcolor size=%superq(ptabsize&z) family="%superq(ptabfamily&z)");
                                                                        %end;
                                            ...
                                            %if %qupcase(%superq(risklabellocation&z))=ABOVE and
                                                ...
                                                layout overlay / border=false walldisplay=none
                                                    ...
                                                    drawtext textattrs=(size=%superq(ptabsize&z) family="%superq(ptabfamily&z)" 
                                                        ...
                                                        %else %do;
                                                             color=&fontcolor
                                                        %end;) %unquote("%superq(class_&z._&i) &sfx2.") /
                                                        y=50  
    ...
    %if &summary=1 %then %do;
        ...
        proc sql noprint;
            ...
            /*Statistics*/
            %local _tndisplay_stat statcolumns _display_current _med_check _hr_check _tl_check _tlmv_check _pval_check _pvalmv_check _covpval_check _covpvalmv_check;
            %let _pval_check=0;
            %let _pvalmv_check=0;
            %let _covpval_check=0;
            %let _covpvalmv_check=0;
            %let _tndisplay_stat=0;        
            %let statcolumns=TOTAL|EVENT|MEDIAN|TIMELIST|EV_N|N_EV|TOTALMV|EVENTMV|EV_NMV|N_EVMV|HR|MEDIANMV|TIMELISTMV|HRMV|PVAL|PVALMV|COVPVAL|COVPVALMV|CINDEX|CINDEXMV|RMST|RMTL|RMST_PVAL;          
            /*Statistics*/
        ...
        data _report;
            ...
            keep modelnum 
               ...
               %end;
               modeltype classlevel subind subtitle 
                ...
                proc report data=_report 
                    ...
                    columns
                        (modelnum /**Used for sorting and distinguishing Models**/
                            ...
                            %end;
                            modeltype /**Used to determine if KM or CIF**/
                            classlevel subind subtitle /*These are always shown*/
                            /*Statistics*/ 
                            %do i = 1 %to &_tndisplay_stat;
                                %superq(_tndisplay_stat_&i)
                            %end;);
                    ...
                    compute &&_tndisplay_stat_&_tndisplay_stat;
                        %if &tableshading=1 %then %do;
                            ...
                            if subind=1 then shade=1;
                                ...
                                if _subind=1 then shade+1;
                                else if ^missing(subtitle) then shade+1;
                            end;
                            _subind=subind;
