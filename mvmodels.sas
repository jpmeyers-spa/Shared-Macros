...
| VERSION UPDATES:
| 2.2 09/27/2021
|   Added TIFFDEVICE as a macro parameter.  This parameter was mistakenly
       missing previously.
| 2.1 08/07/2021
|   Corrected the shading when SHADING=3.
| 2.0 02/17/2021
|   PLOT_DISPLAY and TABLE_DISPLAY replace with singular DISPLAY option
|   SURV_ADJ_METHOD parameter is added
|   PVAL_INTER parameter added and value added to DISPLAY list
|      -only available when SHOW_ADJCOVARIATES=0 and BY is not missing
|   TIME_UNITS added and time-points automatically show up in KM estimates
|   Different SURV_METHODs and SURV_ADJ_METHODs will show up in the table
|      as subheaders and/or footnotes
| 1.3 04/10/2020
|   Corrected coding issues with logistic procedure methods
|   Added catch for logistic not producing type 3 p-values with only continuous
       covariates
|   Corrected indenting issues when using defaults
| 1.2 04/09/2020
|   Fixed error checking on EV_VL and CEN_VL
|   Fixed error with specifying MED_EST_RANGE in CIF survival models
| 1.1 10/23/2018
...
|   DISPLAY = Determines which statistics are displayed in the plot and table.  Items are listed
|             separated by spaces and will show in the order they are listed.  Default=STANDARD.
|             Items that only pertain to the forest plot will be ignored for the table.
|             Options vary by METHOD and are: 
|             All Methods: STANDARD TOTAL EVENTS EV_T PCT EV_T_PCT PVAL PVAL_INTER AIC BIC LOGLIKELIHOOD
...
|         CAT_PARAM = Determines the parameterization of categorical covariates.  Options are REF or EFFECTS. Default is REF.  Applied 
|                     globally to all covariates and models
...
|         SURV_ADJ_METHOD = Method used for adjusted survival. Only applies when SURV_METHOD=KM and when multiple covariates are listed with at least
|                           one categorical covariate.  Options are UNADJUST, DIRECT, and INVWTS.  See reference section for description of
|                           DIRECT and INVWTS methods.
|                           Default is AUTO which will pick the best option given parameters.  If number covariates>2 with at least one categorical covariate
|                           then default will change to INVWTS.  If one or less covariate, or only continuous covariates, or SURV_METHOD=CIF then UNADJUST is selected.
|                           DIRECT will take much more time to calculate than INVWTS or UNADJUST.
...
|                    Default is 1.
|       PVAL_INTER = Determines if a type-3 p-value from interaction model is shown.  The p-value represents the interaction term between 
|                    the BY variable and the first covariate adjusted for any other covariates.  Comes from a separate model than other output.
|                    Options are:
|                    0: No p-value is calculated
|                    1: Wald p-value is calculated
|                    2: Score p-value is calculated (Note: Survival Only)
|                    3: Likelihood-Ratio p-value is calculated (Note: Survival Only)
|                    Default is 1.
|       PVAL_BY = Determines if a p-value is computed comparing the levels of the BY variable.  Is not calcualted when COVARIATES is not missing.
|                 Options are:
|                 0: No p-value is calculated
|                 Survival Methods:
...
%mvmodels (DATA=bmt,METHOD=SURVIVAL,NMODELS=1,
    ...
    DISPLAY=subtitle hr_plot hr_est_range km_est_range1,PLOT_COLUMNWEIGHTS=0.2 0.3 0.25 0.25);  
|
|Survival Example 5: 
%mvmodels (DATA=bmt,METHOD=SURVIVAL,NMODELS=1,
    TIME=ftime,CENS=status,COVARIATES=diagnosis,TYPE=2,COLBY=gender,SHOWWALLS=0,UNDERLINEHEADERS=1,
    TIMELIST=1000,CAT_DISPLAY=4,SHOW_MODELSTATS=0,
    DISPLAY=subtitle hr_plot hr_est_range km_est_range1,PLOT_COLUMNWEIGHTS=0.25 0.3 0.225 0.225,
    width=10in);  
|
|Survival Example 6: 
%mvmodels (DATA=bmt,METHOD=SURVIVAL,NMODELS=1,
...
%mvmodels (DATA=bmt,METHOD=SURVIVAL,NMODELS=2,
    ...
    DISPLAY=subtitle ev_t hr_plot hr_est_range med_est_range,PLOT_COLUMNWEIGHTS=0.2 0.1 0.3 0.2 0.2,
...
%mvmodels (DATA=neuralgia,METHOD=LOGISTIC,NMODELS=1,
    ...
    BY=TREATMENT,
    PVAL_TYPE3=1,DISPLAY=subtitle or_plot ev_t or_est_range pval,PLOT_COLUMNWEIGHTS=0.2 0.3 0.15 0.2 0.15);
|
|Logistic Example 5: 
%mvmodels (DATA=neuralgia,METHOD=LOGISTIC,NMODELS=1,
    EVENTCOV=pain,EVENT=Yes,COVARIATES=sex age,TYPE=2 1,
...
|Logistic Example 6: 
%mvmodels (DATA=neuralgia,METHOD=LOGISTIC,NMODELS=1,
    EVENTCOV=pain,EVENT=Yes,COVARIATES=treatment sex age,TYPE=2 2 1,CAT_REF=P`M,
...
%macro mvmodels(
    ...
    /*2.0: Optional Parameters*/
      /*2.1: Model Variables Options*/
        ...
        /*2.1.3: Survival Options*/
            ...
            /*2.1.3.2: Methods Options*/
              surv_method=KM,ties=breslow,surv_adj_method=AUTO,
            /*2.1.3.3: Time-point Estimates Options*/
              timelist=,tdivisor=1,time_units=Units,
        ...
        /*2.1.7: P-value Options*/
          pval_covariates=1,pval_type3=1,pval_by=0,pval_priority=covariate,pval_inter=1,pfoot=1,
      /*2.2: Output Display Options*/
        /*2.2.1: Output Display Options*/
          ...
          /*2.2.1.2: Column Heading Options*/
            ...
            pvalheader=P-value,pval_interheader=Interaction`P-value,
            ...
            km_estimateheader=,km_est_rangeheader=,km_rangeheader=,
            ref_km_estimateheader=,ref_km_est_rangeheader=,ref_km_rangeheader=,
        ...
        /*2.2.2: Table Display Options*/
          ...
          /*2.2.2.2: Column Width Options */
            ...
            pvalwidth=0.7in,pval_interwidth=0.7in,
            ...
            dummywidth=0.1in,
            AICwidth=0.7in,bicwidth=0.7in,loglikelihoodwidth=0.7in,
        /*2.2.3: Plot Display Options*/
          /*2.2.3.1: General Options*/
      ...
      /*2.5: Output options*/
        /*2.05: Image File Output Settings*/
          ...
          plottype=static,svg=0,tiffdevice=TIFFP,transparent=0,width=8in,
    ...
    /*Section 1.1.3: Error Handling on Model Numeric Variables*/
        ...
        %macro _numcheck(parm,min,contain,default,maxvals=1,require=0);
            /**Check if missing**/
            %local j;
            ...
            %else %if &require=1 %then %do;
                /**Check if value is below minimum threshold**/
                %put ERROR: (Model &i: %qupcase(&parm)) value cannot be missing;
                %let nerror=%eval(&nerror+1);
            %end;
    ...
    %_gparmcheck(cat_param,REF|EFFECT)
    ...
    %end;        
    %_gparmcheck(surv_adj_method,AUTO|UNADJUST|DIRECT|INVWTS)   
    ...
    %_gunitcheck(loglikelihoodwidth)
    ...
    %let _mvarlist=%sysfunc(compress(
            ...
            covariates|type|surv_adj_method|
            pval_covariates|pval_type3|pval_by|pval_priority|pval_inter
        ));
    %local i j l m n o k; 
    ...
    %if &nerror=0 %then %do i = 1 %to &nmodels; 
        ...
        %if %qupcase(&method)=SURVIVAL %then %do;
            ...
            %if %qupcase(&surv_method)^=CIF %then %do;
                ...
                %_parmcheck(pval_inter,0|1|2|3)
                %_parmcheck(pval_by,0|1|2)
            %end;
            %else %do;
                %_parmcheck(pval_type3,0|1)
                %_parmcheck(pval_inter,0|1)
            ...
            %_numcheck(cen_vl,require=1)
            %if %sysevalf(%qupcase(%superq(surv_method))=CIF,boolean) %then %_numcheck(ev_vl,require=1);
    ...
    %let any_by=0;
    %do i=1 %to &nmodels;
        %if %sysevalf(%superq(by&i)^=,boolean) %then %let any_by=1;
    %end;
    %if %sysevalf(%qupcase(&display)=STANDARD,boolean) %then %do;
        %if %qupcase(&method)=SURVIVAL %then %do;
            %let display=subtitle;
            %if %sysevalf(%sysfunc(compress(%superq(covariates),|))^=,boolean) %then %do;
                %let display=&display hr_plot;
                %if %sysfunc(find(&show_modelstats,1))>0 or %sysfunc(find(&show_modelstats,2))>0 or
                    (%sysevalf(%superq(covariates)^=,boolean) and %sysfunc(find(%superq(type),2))>0
                     and %sysfunc(find(%superq(cat_display),4))>0) %then %let display=&display ev_t hr_est_range;
                %else %let display=&display hr_est_range;
                %if %sysevalf(%superq(timelist)^=,boolean) %then %do i = 1 %to %sysfunc(countw(%superq(timelist),%str( )));
                    %let display=&display km_est_range&i;
                %end;
                %if %sysevalf(%sysfunc(compress(%superq(pval_covariates),0|))^=,boolean) or
                    %sysevalf(%sysfunc(compress(%superq(pval_type3),0|))^=,boolean) %then
                    %let display=&display pval;  
            %end;
            %else %do;
                %if %sysevalf(%superq(timelist)^=,boolean) %then %do i = 1 %to %sysfunc(countw(%superq(timelist),%str( )));
                    %if &i=1 and %sysfunc(find(&show_modelstats,1))>0 %then %let display=&display km_plot&i ev_t;
                    %else %let display=&display km_plot&i;
                    %let display=&display km_est_range&i;
                %end; 
                %else %if %sysfunc(find(&show_modelstats,1))>0 %then %let display=&display med_plot ev_t med_est_range;
                %else %let display=&display med_plot med_est_range;
                %if %sysevalf(%sysfunc(compress(%superq(pval_by),0|))^=,boolean) and &any_by=1 %then
                    %let display=&display pval;                
            %end;
        %end;
        %else %if %qupcase(&method)=LOGISTIC %then %do;
            %let display=subtitle;
            %if %sysevalf(%sysfunc(compress(%superq(covariates),|))^=,boolean) %then %do;
                %let display=&display or_plot;
                %if %sysfunc(find(&show_modelstats,1))>0 %then %let display=&display ev_t or_est_range;
                %else %let display=&display or_est_range;
                %if %sysevalf(%sysfunc(compress(%superq(pval_covariates),0|))^=,boolean) or
            ...
            %end;
            %else %do;
                %if %sysfunc(find(&show_modelstats,1))>0 %then %let display=&display bin_plot ev_t bin_est_range;
                %else %let display=&display bin_plot bin_est_range;
    ...
    %else %if &nerror=0 %then %do;
        ...
        %if %qupcase(&method)=SURVIVAL %then %do;
            ...
            %if %sysevalf(%sysfunc(compress(%superq(covariates),|))^=,boolean) %then %do;
                ...
                %if %sysfunc(find(%superq(pval_type3),1)) or %sysfunc(find(%superq(pval_type3),2)) or %sysfunc(find(%superq(pval_type3),3)) or
                    %sysfunc(find(%superq(pval_covariates),1)) %then %let list=&list|PVAL;
                %if (%sysfunc(find(%superq(pval_inter),1)) or %sysfunc(find(%superq(pval_inter),2)) or %sysfunc(find(%superq(pval_inter),3))) and
                    &any_by=1 %then %let list=&list|PVAL_INTER;
            %end;
            %if %sysevalf(%superq(timelist)^=,boolean) %then %do i = 1 %to %sysfunc(countw(%superq(timelist),%str( )));
                ...
                %end;
            %end;
            %if %sysfunc(find(%superq(pval_by),1)) or %sysfunc(find(%superq(pval_by),2)) and &any_by=1 %then %let list=&list|PVAL;
                
            %_glistcheck(var=display,_patternlist=&list,lbl=display,msg=)
            %if %sysfunc(find(&display,C_,i))>0 and %qupcase(&surv_method)=CIF %then
        ...
        %else %if %qupcase(&method)=LOGISTIC %then %do;
            ...
            %if %sysevalf(%sysfunc(compress(%superq(covariates),|))^=,boolean) %then %do;
                ...
                %if %sysfunc(find(%superq(pval_inter),1)) and &any_by=1 %then %let list=&list|PVAL_INTER;
            %end;
            %if %sysfunc(find(%superq(pval_by),1)) or %sysfunc(find(%superq(pval_by),2)) or 
                %sysfunc(find(%superq(pval_type3),1)) or 
                %sysfunc(find(%superq(pval_covariates),1)) %then %let list=&list|PVAL;
            %_glistcheck(var=display,_patternlist=&list,lbl=display,msg=)
            %if %sysfunc(find(&display,C_,i))>0 and %sysevalf(%sysfunc(compress(%superq(strata),%str( |)))^=,boolean) %then
                %put WARNING: C-index cannot be calculated from a stratified logistic model.;
        %end;
    %end;

    %if %sysevalf(%superq(colby)^=,boolean) and %sysevalf(%qupcase(%superq(colsubtitles))=END,boolean) and %sysfunc(find(&display,subtitle,i))>0 %then %do;
        %let display=%sysfunc(tranwrd(%qupcase(%superq(display)),SUBTITLE,%str())) SUBTITLE;
    %end;
    %else %if %sysevalf(%superq(colby)^=,boolean) and %sysevalf(%qupcase(%superq(colsubtitles))=START,boolean) and %sysfunc(find(&display,subtitle,i))>0 %then %do;
        %let display=SUBTITLE %sysfunc(tranwrd(%qupcase(%superq(display)),SUBTITLE,%str()));
    %end;

    %local _nplot_display _nplot _nplot_pre _ntable_display ;
    %let _nplot=0;%let _nplot_pre=;
    %let _nplot_display=%sysfunc(countw(&display,%str( )));
    %let _ntable_display=0;
    /**Create macro variable Triggers For Different Statistics**/
    %local _sum _median _km _cindex _pval _interp _hr _fit _bin;
    %local _nsubtitle;
    %let _nsubtitle=0;
    %if &nerror = 0 %then %do j = 1 %to %superq(_nplot_display);
        %local _plot_display&j. _plot_displayheader&j. _table_display&j. _table_displayheader&j.;

        %if %qupcase(%scan(%superq(display),&j,%str( )))=EV_T or
            %qupcase(%scan(%superq(display),&j,%str( )))=PCT or
            %qupcase(%scan(%superq(display),&j,%str( )))=EV_T_PCT or
            %qupcase(%scan(%superq(display),&j,%str( )))=EVENTS or
            %qupcase(%scan(%superq(display),&j,%str( )))=TOTAL %then %let _sum=1;
        %else %if %qupcase(%scan(%superq(display),&j,%str( )))=REF_EV_T or
            %qupcase(%scan(%superq(display),&j,%str( )))=REF_PCT or
            %qupcase(%scan(%superq(display),&j,%str( )))=REF_EV_T_PCT or
            %qupcase(%scan(%superq(display),&j,%str( )))=REF_EVENTS or
            %qupcase(%scan(%superq(display),&j,%str( )))=REF_TOTAL %then %let _sum=1;
        %else %if %qupcase(%scan(%scan(%superq(display),&j,%str( )),1,_))=MED %then %let _median=1;
        %else %if %qupcase(%scan(%scan(%superq(display),&j,%str( )),2,_))=MED %then %let _median=1;
        %else %if %qupcase(%scan(%scan(%superq(display),&j,%str( )),1,_))=BIN %then %let _bin=1;
        %else %if %qupcase(%scan(%scan(%superq(display),&j,%str( )),1,_))=KM %then %let _km=1;
        %else %if %qupcase(%scan(%scan(%superq(display),&j,%str( )),2,_))=KM %then %let _km=1;
        %else %if %qupcase(%scan(%scan(%superq(display),&j,%str( )),1,_))=C %then %let _cindex=1;
        %else %if %qupcase(%scan(%scan(%superq(display),&j,%str( )),1,_))=HR or 
            %qupcase(%scan(%scan(%superq(display),&j,%str( )),1,_))=%str(OR) %then %let _hr=1;
        %else %if %qupcase(%scan(%superq(display),&j,%str( )))=PVAL_INTER %then %let _interp=1;
        %else %if %qupcase(%scan(%superq(display),&j,%str( )))=PVAL %then %let _pval=1;
        %else %if %qupcase(%scan(%superq(display),&j,%str( )))=AIC or
            %qupcase(%scan(%superq(display),&j,%str( )))=BIC or
            %qupcase(%scan(%superq(display),&j,%str( )))=LOGLIKELIHOOD %then %let _fit=1;
        
        %if %qupcase(%scan(%superq(display),&j,%str( )))=SUBTITLE %then %do;
            %let _plot_display&j.=subtitle;
            %let _plot_displayheader&j.=%superq(subtitleheader);
            %let _nsubtitle=&j;
        %end;
        %else %if %sysfunc(find(%scan(%superq(display),&j,%str( )),PLOT,i))>0 %then %do;
            %let _plot_display&j.=%scan(%superq(display),&j,%str( ));
            %let _nplot=%sysevalf(%superq(_nplot)+1);
            %let _plot_displayheader&j.=%qscan(%superq(plot_header),&_nplot,|,m);
            %let _nplot_pre=%sysfunc(strip(%superq(_nplot_pre)))|%qupcase(%sysfunc(strip(%scan(%scan(%superq(display),&j,%str( )),1,_))));
        %end;
        %else %if %sysfunc(find(%scan(%superq(display),&j,%str( )),REF_KM_,i))>0 %then %do;
            %let _plot_display&j.=%scan(%superq(display),&j,%str( ));
            %local k;
            %let k=%sysfunc(compress(%qupcase(%scan(%superq(display),&j,%str( ))),REFKM_ESTIMATELCLUCLRANGE,i));
            %let kmtime=%qscan(%superq(timelist),&k,%str( ));
            %if %qupcase(%scan(%superq(display),&j,%str( )))=REF_KM_ESTIMATE&k %then %do;
                %let _plot_displayheader&j.=%qscan(%superq(REF_KM_ESTIMATEheader),&k,|,m);
                %if %sysevalf(%superq(_plot_displayheader&j)=,boolean) %then %let _plot_displayheader&j.=Reference`&kmtime &time_units`Event-free Rate;
                %let _ntable_display=%sysevalf(&_ntable_display+1);
                %let _table_display&_ntable_display.=%scan(%superq(display),&j,%str( ));
                %let _table_displayheader&_ntable_display.=&&_plot_displayheader&j.;
            %end;
            %else %if %qupcase(%scan(%superq(display),&j,%str( )))=REF_KM_LCL&k %then %do;
                %let _plot_displayheader&j.=%qscan(%superq(REF_KM_LCLheader),&k,|,m); 
                %if %sysevalf(%superq(_plot_displayheader&j)=,boolean) %then %let _plot_displayheader&j.=Reference`&kmtime &time_units`Event-free Rate LCL;
                %let _ntable_display=%sysevalf(&_ntable_display+1);
                %let _table_display&_ntable_display.=%scan(%superq(display),&j,%str( ));
                %let _table_displayheader&_ntable_display.=&&_plot_displayheader&j.;
            %end;
            %else %if %qupcase(%scan(%superq(display),&j,%str( )))=REF_KM_UCL&k %then %do;
                %let _plot_displayheader&j.=%qscan(%superq(REF_KM_UCLheader),&k,|,m); 
                %if %sysevalf(%superq(_plot_displayheader&j)=,boolean) %then %let _plot_displayheader&j.=Reference`&kmtime &time_units`Event-free Rate UCL;
                %let _ntable_display=%sysevalf(&_ntable_display+1);
                %let _table_display&_ntable_display.=%scan(%superq(display),&j,%str( ));
                %let _table_displayheader&_ntable_display.=&&_plot_displayheader&j.;
            %end;
            %else %if %qupcase(%scan(%superq(display),&j,%str( )))=REF_KM_RANGE&k %then %do;
                %let _plot_displayheader&j.=%qscan(%superq(REF_KM_RANGEheader),&k,|,m);
                %if %sysevalf(%superq(_plot_displayheader&j)=,boolean) %then %let _plot_displayheader&j.=Reference`&kmtime &time_units`Event-free Rate 95% CI; 
                %let _ntable_display=%sysevalf(&_ntable_display+1);
                %let _table_display&_ntable_display.=%scan(%superq(display),&j,%str( ));
                %let _table_displayheader&_ntable_display.=&&_plot_displayheader&j.;
            %end;
            %else %if %qupcase(%scan(%superq(display),&j,%str( )))=REF_KM_EST_RANGE&k %then %do;
                %let _plot_displayheader&j.=%qscan(%superq(REF_KM_EST_RANGEheader),&k,|,m); 
                %if %sysevalf(%superq(_plot_displayheader&j)=,boolean) %then %let _plot_displayheader&j.=Reference`&kmtime &time_units`Event-free Rate`(95% CI);
                %let _ntable_display=%sysevalf(&_ntable_display+1);
                %let _table_display&_ntable_display.=%scan(%superq(display),&j,%str( ));
                %let _table_displayheader&_ntable_display.=&&_plot_displayheader&j.;
            %end;
        %end;
        %else %if %sysfunc(find(%scan(%superq(display),&j,%str( )),KM_,i))>0 %then %do;
            %let _plot_display&j.=%scan(%superq(display),&j,%str( ));
            %local k;
            %let k=%sysfunc(compress(%qupcase(%scan(%superq(display),&j,%str( ))),REFKM_ESTIMATELCLUCLRANGE,i));
            %let kmtime=%qscan(%superq(timelist),&k,%str( ));
            %if %qupcase(%scan(%superq(display),&j,%str( )))=KM_ESTIMATE&k %then %do;
                %let _plot_displayheader&j.=%qscan(%superq(KM_ESTIMATEheader),&k,|,m);
                %if %sysevalf(%superq(_plot_displayheader&j)=,boolean) %then %let _plot_displayheader&j.=&kmtime &time_units`Event-free Rate;
                %let _ntable_display=%sysevalf(&_ntable_display+1);
                %let _table_display&_ntable_display.=%scan(%superq(display),&j,%str( ));
                %let _table_displayheader&_ntable_display.=&&_plot_displayheader&j.;
            %end;
            %else %if %qupcase(%scan(%superq(display),&j,%str( )))=KM_LCL&k %then %do;
                %let _plot_displayheader&j.=%qscan(%superq(KM_LCLheader),&k,|,m); 
                %if %sysevalf(%superq(_plot_displayheader&j)=,boolean) %then %let _plot_displayheader&j.=&kmtime &time_units`Event-free Rate LCL;
                %let _ntable_display=%sysevalf(&_ntable_display+1);
                %let _table_display&_ntable_display.=%scan(%superq(display),&j,%str( ));
                %let _table_displayheader&_ntable_display.=&&_plot_displayheader&j.;
            %end;
            %else %if %qupcase(%scan(%superq(display),&j,%str( )))=KM_UCL&k %then %do;
                %let _plot_displayheader&j.=%qscan(%superq(KM_UCLheader),&k,|,m); 
                %if %sysevalf(%superq(_plot_displayheader&j)=,boolean) %then %let _plot_displayheader&j.=&kmtime &time_units`Event-free Rate UCL;
                %let _ntable_display=%sysevalf(&_ntable_display+1);
                %let _table_display&_ntable_display.=%scan(%superq(display),&j,%str( ));
                %let _table_displayheader&_ntable_display.=&&_plot_displayheader&j.;
            %end;
            %else %if %qupcase(%scan(%superq(display),&j,%str( )))=KM_RANGE&k %then %do;
                %let _plot_displayheader&j.=%qscan(%superq(KM_RANGEheader),&k,|,m);
                %if %sysevalf(%superq(_plot_displayheader&j)=,boolean) %then %let _plot_displayheader&j.=&kmtime &time_units`Event-free Rate 95% CI; 
                %let _ntable_display=%sysevalf(&_ntable_display+1);
                %let _table_display&_ntable_display.=%scan(%superq(display),&j,%str( ));
                %let _table_displayheader&_ntable_display.=&&_plot_displayheader&j.;
            %end;
            %else %if %qupcase(%scan(%superq(display),&j,%str( )))=KM_EST_RANGE&k %then %do;
        ...
        %else %do;
            %let _plot_display&j.=%scan(%superq(display),&j,%str( ));
            %let _plot_displayheader&j.=%superq(%scan(%superq(display),&j,%str( ))header); 
            %let _ntable_display=%sysevalf(&_ntable_display+1);
            %let _table_display&_ntable_display.=%scan(%superq(display),&j,%str( ));
            %let _table_displayheader&_ntable_display.=%superq(%scan(%superq(display),&j,%str( ))header);
        %end;
    %end;
    ...
    %do i = 1 %to &_nplot;
        /**Cycle through each macro parameter**/
        %do j = 1 %to %sysfunc(countw(&_pvarlist,|,m));
            %local v&j;
            %let v&j=%scan(%superq(_pvarlist),&j,|,m);
            %local &&v&j..&i;
            /**If the | delimiter is detected, assign the different values between | to numbered parameters**/
            /**Else Assign the same value to all numbered parameters**/
            %if %index(%superq(&&v&j),|)>0 %then %let &&v&j..&i=%scan(%superq(&&v&j),&i,|,m);
    ...
    %if %sysevalf(%qupcase(%superq(method))=SURVIVAL,boolean) and &nerror=0 %then %do;
        %if &_interp=1 %then %do;
            %if %sysevalf(%superq(by&i)^=,boolean) and &&pval_inter&i>0 and &&show_adjcovariates&i=1 %then %do;
                %put ERROR: (Model &i: PVAL_INTER): SHOW_ADJCOVARIATES must be set to 0 to display interaction p-values;
                %let nerror=%eval(&nerror+1);   
            %end;
        %end;
        /**Check adjusting survival methods**/
        %do i = 1 %to &nmodels;
            %if %sysfunc(countw(%superq(covariates&i),%str( )))>=2 %then %do;
                %if %qscan(&&type&i,1,%str( ))=1 %then %do; 
                    %local _catcheck;
                    %let _catcheck=0;
                    %do j=1 %to %sysfunc(countw(%superq(covariates&i),%str( )));
                        %if %qscan(&&type&i,&j,%str( ))=%str(2) %then %let _catcheck=1;
                    %end;
                    %if &&show_adjcovariates&i=0 or _catcheck=0 %then %do;
                        %if %qupcase(%superq(surv_adj_method&i))=AUTO or %qupcase(%superq(surv_adj_method&i))=UNADJUST %then %let surv_adj_method&i=0;
                        %else %do;  
                            %put ERROR: (Model &i: SURV_ADJ_METHOD): Cannot be set to %qupcase(&&surv_adj_method&i) with only continuous covariates;
                            %let nerror=%eval(&nerror+1);                    
                        %end;
                    %end;
                    %else %if %qupcase(%superq(surv_adj_method&i))=AUTO or %qupcase(%superq(surv_adj_method&i))=DIRECT %then %let surv_adj_method&i=1;
                    %else %if %qupcase(%superq(surv_adj_method&i))=INVWTS %then %let surv_adj_method&i=2;
                    %else %if %qupcase(%superq(surv_adj_method&i))=UNADJUST %then %let surv_adj_method&i=0;
                %end;           
                %else %do;
                    %if %qupcase(%superq(surv_adj_method&i))=AUTO or %qupcase(%superq(surv_adj_method&i))=DIRECT %then %let surv_adj_method&i=1;
                    %else %if %qupcase(%superq(surv_adj_method&i))=INVWTS %then %let surv_adj_method&i=2;
                    %else %if %qupcase(%superq(surv_adj_method&i))=UNADJUST %then %let surv_adj_method&i=0;
                %end;
            %end;
            %else %do;  
                %if %qupcase(%superq(surv_adj_method&i))=AUTO or %qupcase(%superq(surv_adj_method&i))=UNADJUST %then %let surv_adj_method&i=0;
                %else %do;  
                    %put ERROR: (Model &i: SURV_ADJ_METHOD): Cannot be set to %qupcase(&&surv_adj_method&i) when less than two covariates are listed;
                    %let nerror=%eval(&nerror+1);                    
                %end;
            %end;
        %end;
    ...
    %if %sysevalf(%superq(rowby)^=,boolean) or %sysevalf(%superq(colby)^=,boolean) or %sysevalf(%superq(groupby)^=,boolean) %then %do;
        ...
        data _temp_group;
            ...
            %if %sysevalf(%superq(rowby)^=,boolean) %then %do;
                ...
                else if ^(strip(vvalue(_temp_rowby))='.' and _temp_Rowby_=.) then _rowby_=strip(vvalue(_temp_rowby));
            %end;
            %if %sysevalf(%superq(colby)^=,boolean) %then %do;
                length _colby_ $300.;
                ...
                else if ^(strip(vvalue(_temp_colby))='.' and _temp_colby_=.) then _colby_=strip(vvalue(_temp_colby));
            %end;
            %if %sysevalf(%superq(groupby)^=,boolean) %then %do;
                ...
                else if ^(strip(vvalue(_temp_groupby))='.' and _temp_groupby_=.) then _groupby_=strip(vvalue(_temp_groupby));
            %end;
        run;
    ...
    %do i = 1 %to &nmodels;
        proc sql;
            create table _models&i
                (modelnum num,modelsum_row num,show_modelstats num, show_adjcovariates num, 
                %if %sysevalf(%qupcase(&method)=SURVIVAL,boolean) %then %do; 
                    surv_method char(20),surv_adj_method char(20),
                %end;
                nstrata num, ncovariates num, ncat num, ncont num, 
                ...
                pval_by num, pval_type3 num, pval_covariates num, pval_inter num, added_row num);
            %if %sysevalf(%superq(strata&i)^=,boolean) %then %do;
                alter table _models&i
                    add 
        ...
        %end;
        %local time_label&i;
        %if %qupcase(&method)=SURVIVAL %then %let time_label&i=%sysfunc(varlabel(&_data,%sysfunc(varnum(&_data,&&time&i))));
        ...
        run;
        %if (&&_ncat&i > 1 and &&show_adjcovariates&i=0) or (%scan(&&type&i,1,%str( ))=1 and &&_ncat&i>=1 and &&show_adjcovariates&i=0) %then %do;
            data _tempdsn&i;
                set _tempdsn&i;
                by modelnum _rowby_lvl _colby_lvl _groupby_lvl _by_num _by_lvl;
    ...
    %do i = 1 %to &nmodels;
        proc sql noprint;
            ...
            %if &&_ncov&i=1 and %scan(&&type&i,1,%str( ))=1 and &&model_title_ownrow&i=0 %then %let _vmerge=1;
            %else %if &&_ncov&i=1 and %scan(&&type&i,1,%str( ))=2 and &&model_title_ownrow&i=0 %then %let _vmerge=2;
            %else %let _vmerge=0;
            insert into _models&i
                %do r = 1 %to &_rows;
                    %do c = 1 %to &_columns;
                        %do g = 1 %to &_groups;
                            %if %sysevalf(%superq(model_title&i)^=,boolean) or
                                (&_vmerge^=0 and %sysevalf(%superq(by&i)=,boolean)) or 
                                (&nmodels>1 and %sysevalf(%superq(by&i)=,boolean)) or 
                                (%sysevalf(%superq(by&i)=,boolean) and %sysevalf(%superq(model_title&i)=,boolean) and &&_ncov&i=0) %then %do;
                            set modelnum=&i,
                                subtitle=
                                case
                                   ...
                                   %if %sysevalf(%superq(by&i)=,boolean) %then %do;
                                       %if &&_ncov&i=0 %then %do;
                                           when ^missing("%superq(time_label&i)") then "%superq(time_label&i)"
                                       %end;
                                ...
                                %if %sysevalf(%superq(model_title&i)^=,boolean) or &_vmerge=0 %then %do;
                                    subind=coalesce(&&indent_model_title&i,0),
                                    boldind=coalesce(&&bold_model_title&i,1),
                                %end;
                                %else %do;
                                    subind=coalesce(&&indent_cov_label&i,0),
                            ...
                            %do b = 1 %to %sysfunc(max(1,%sysfunc(countw(%superq(by&i),%str( )))));
                                %if %sysevalf(%superq(by&i)^=,boolean) and &&bylabelon&i=1 %then %do;
                                    set modelnum=&i,
                                        ...
                                        pval_by=&&pval_by&i,pval_inter=&&pval_inter&i,
                                        ...
                                        %if %sysevalf(%scan(&&type&i,&v,%str( ))=1,boolean) %then %do;
                                            ...
                                            %if &_vmerge=0 %then %do;
                                                set modelnum=&i,
                                                    ...
                                                    subind=coalesce(&&indent_cov_label&i,((%sysevalf(%superq(model_title&i)^=,boolean) or (&nmodels>1 and %sysevalf(%superq(by&i)=,boolean))) and &_vmerge=0)+
                                                         %if %sysevalf(%superq(by&i)^=,boolean) %then %do; &&bylabelon&i+ %end; %sysevalf(%superq(by&i)^=,boolean)),
                                                    boldind=coalesce(&&bold_cov_label&i,1),
                                                    nstrata=%sysfunc(countw(%superq(strata&i),%str( ))),ncat=&&_ncat&i,ncont=&&_ncont&i,ncovariates=&&_ncat&i+&&_ncont&i,
                                                    show_adjcovariates=&&show_adjcovariates&i,
                                            ...
                                            %else %do; , %end;
                                                ...
                                                varnum=&v,varlevel=ifn(&&cont_display&i=3,0,1)
                                                %if &&cont_display&i^=3 %then %do; ,cont_step=%superq(_cont_step&i._&_cont_indx) %end;

                                            %if &&cont_display&i=3 %then %do;
                                                set modelnum=&i,
                                                    subtitle="Step Size: "||strip(put(%superq(_cont_step&i._&_cont_indx),12.)),
                                                    subind=coalesce(&&indent_cov_value&i,1+((%sysevalf(%superq(model_title&i)^=,boolean) or (&nmodels>1 and %sysevalf(%superq(by&i)=,boolean))) and &_vmerge=0)+
                                                         %if %sysevalf(%superq(by&i)^=,boolean) %then %do; &&bylabelon&i+ %end; %sysevalf(%superq(by&i)^=,boolean)),
                                                    boldind=coalesce(&&bold_by_value&i,0),
                                                    nstrata=%sysfunc(countw(%superq(strata&i),%str( ))),ncat=&&_ncat&i,ncont=&&_ncont&i,ncovariates=&&_ncat&i+&&_ncont&i,
                                                    show_adjcovariates=&&show_adjcovariates&i,
            ...
            %if %sysevalf(%qupcase(&method)=SURVIVAL,boolean) %then %do; 
                update _models&i
                    set surv_method="&&surv_method&i",
                        surv_adj_method=case (&&surv_adj_method&i)
                            when 0 then "Unadjust"
                            when 1 then 'DIRECT'
                            when 2 then 'INVWTS'
                        else '' end;
            %end;
        ...
        data _tempdsn&i;
            set _tempdsn&i (in=a&i);
            by modelnum _rowby_lvl _colby_lvl _groupby_lvl _by_num _by_lvl;
            if first._by_lvl then model+1;
        run;
        proc sql noprint;
            %local _models&i;
            select distinct model into :_models&i separated by ' ' from _tempdsn&i;
            ...
            %if &nerror_run=0 and %sysevalf(%qupcase(%superq(method))=LOGISTIC,boolean) %then %do;
                ...
                select max(model) into :nmodel_lvls&i separated by '' from _tempdsn&i;
                select distinct model into :models&i separated by ' ' from _tempdsn&i;
                %if %sysevalf(%superq(event&i)=,boolean) %then %do;
                    select min(_event_) into :event&i separated by '' 
                        from _tempdsn&i where modelnum=&i; 
                %end;
                update _tempdsn&i
                ...
                select max(_event_val),min(count),max(count) into :ev_val_check separated by '',:min_nev separated by'',:max_nev separated by ''
                    from (select model,max(_event_val) as _event_val,count(distinct _event_) as count from _tempdsn&i where modelnum=&i group by model);
            ...
            /**Transpose Models**/
            %local max_cat_lvl;
            select max(ifn(missing(varlevel),0,varlevel)) into :max_cat_lvl from _models&i;
            create table _legit_models&i as
        ...
        data _models&i;
            merge _models&i _legit_models&i;
            by modelnum rowby_lvl colby_lvl groupby_lvl by_num by_lvl;
            if missing(model) and (added_row>0 or varnum=-10 or (by_num>0 and by_lvl=0)) then model=0;
            if ^missing(model) then output;
        run;
       
        %local nstrata&i;
        %let nstrata&i=%sysfunc(countw(%superq(strata&i),%str( )));
        
        data _tempdsn&i._p2;
            length _varname_ $40. _varval_ $300.;
            set %if &&_ncat&i=0 %then %do; _tempdsn&i (in=all); %end;
                %else %do k = 1 %to %sysfunc(countw(&&_models&i,%str( )));
                    _tempdsn&i (in=all&k where=(model=%scan(&&_models&i,&k,%str( )))) 
                    %do j = 1 %to &&_ncat&i;
                        _tempdsn&i (in=cat&i._&j where=(model=%scan(&&_models&i,&k,%str( ))) rename=(_cat_lvl_&j=_varlvl_ _cat_&j=_varval_))
                    %end;
                %end;;
            %if &&_ncat&i=0 %then %do; _varname_='_all_';_varval_='all';_varlvl_=.; %end;
            %else %do k = 1 %to %sysfunc(countw(&&_models&i,%str( )));
                %if &k>1 %then %do; else %end;
                if all&k then do;_varname_='_all_';_varval_='all';end;
                %do j = 1 %to &&_ncat&i;
            ...
            %end;
        run;
        proc sort data=_tempdsn&i._p2;
            by model _varname_ _varval_;
        run;
        %if &&_ncat&i>0 %then %do;
            proc sql;
                delete from _models&i
                    where cov_type=2 and varlevel>0 and
                    catx('--',modelnum,rowby_lvl,colby_lvl,groupby_lvl,by_num,by_lvl,covariate,varlevel)
                    ^in(select catx('--',modelnum,_rowby_lvl,_colby_lvl,_groupby_lvl,_by_num,_by_lvl,upcase(_varname_),_varlvl_) from _tempdsn&i._p2);
            quit;
        %end;
        
            
        %if %sysevalf(%qupcase(&method)=LOGISTIC,boolean) and (&_sum=1 or &_bin=1) %then %do;
        ...
        %if %sysevalf(%qupcase(&method)=LOGISTIC,boolean) and 
            %sysevalf(%superq(by&i)^=,boolean) and %sysevalf(%superq(pval_by&i)^=0,boolean)
            and &_pval=1 %then %do;               
            proc sort data=_tempdsn&i._p2 out=_tempdsn&i._p3;
                by modelnum _rowby_lvl _colby_lvl _groupby_lvl _by_num _by_lvl;
            ...
            run; 
        %end;
        %if %sysevalf(%qupcase(&method)=SURVIVAL,boolean) and (&_km=1 or &_sum=1 or &_median=1) %then %do;     
            /**Inverse Weights weight setups**/
            %if &&surv_adj_method&i=2 and (&_km=1 or &_median=1) %then %do;
                proc sql noprint;
                    create table _adjwts_prep1 as
                        select *,count(distinct _varlvl_) as nlvl,count(*) as total
                            from 
                                (select *,count(*) as n from _tempdsn&i._p2
                                    where _varname_^='_all_'
                                    group by model,_varname_,_varlvl_)
                ...
                data _adjwts_prep2;
                    ...
                    do i = 1 to nlvl;
                        _set_=i;
                        if _varlvl_=i then _adjevent_=1;
                        else _adjevent_=0;
                        output;
                    end;
                    drop i;
                ...
                proc sort data=_adjwts_prep2;    
                    by model _varname_ _set_;
                run;
                
                proc logistic data=_adjwts_prep2;
                    by model _varname_ _set_;
                    ...
                    class 
                        ...
                        %do j = 1 %to &&_ncat&i; 
                            _cat_lvl_&j 
                        %end;
                        %do j = 1 %to &&nstrata&i; _strata_&j %end;   / param=&cat_param;  
                    /**Run model**/
                    model _adjevent_ (event='1') = 
                        %do j = 1 %to &&_ncat&i; _cat_lvl_&j %end; /**Class Covariates**/
                        %do j = 1 %to &&_ncont&i; _cont_&j %end; /**Continuous Covariates**/
                        /**Apply strata**/
                        %do j = 1 %to &&nstrata&i;  _strata_&j %end;; 
                    output out=_adjwts prob=_adjwts_;
                run;
                
                data _tempdsn&i._p2;
                    set _tempdsn&i._p2 (in=univ where=(_varname_='_all_'))
                        _adjwts (where=(_set_=_varlvl_));
                        
                    if univ then weight=1;
                    else weight=(n/total)/_adjwts_;
                    drop _set_ _adjevent_;
                run;
                proc sort data=_tempdsn&i._p2;
                    by model _varname_;
            %end;
            %if %sysevalf(%qupcase(%superq(surv_method))^=CIF,boolean) %then %do;
                proc lifetest data=_tempdsn&i._p2
                    %if &_km=1 %then %do;
                    ...
                    %if &&surv_adj_method&i=2 %then %do; weight weight; %end;
                ...
                %if &&surv_adj_method&i=1 and (&_km=1 or &_median=1) %then %do;
                    proc sql noprint;
                        create table _adjwts_prep1 as
                            select *,count(distinct _varlvl_) as nlvl,count(*) as total
                                from 
                    ...
                    proc phreg data=_tempdsn&i._p2 plots(cl)=survival ;
                        by model _varname_;
                        strata _varlvl_;
                        class %do j = 1 %to &&nstrata&i; _strata_&j %end;
                            %do j=1 %to &&_ncat&i; _cat_val_&j %end;;
                        model _time_*_cens_(0)= 
                            %do j=1 %to &&_ncat&i; _cat_val_&j %end; /**Class Covariates**/
                            %do j=1 %to &&_ncont&i; _cont_&j %end; /**Numeric Covariates**/
                            %do j = 1 %to &&nstrata&i; _strata_&j %end; /**Stratification Factors**/ / rl;
                        baseline covariates=_covs out=_surv_adj (rename=(_time_=t1)) survival=s1 lower=lcl1 upper=ucl1/ diradj cltype=&conftype; 
                        where _varname_^='_all_' and missing(_varlvl_)=0
                            %do j=1 %to &&_ncat&i; and missing(_cat_val_&j)=0 %end;
                            %do j=1 %to &&_ncont&i; and missing(_cont_&j)=0 %end;;
                    run;  
                    data 
                        %if &_median=1 %then %do;
                            _quart (keep=model _varname_ _varlvl_ percent estimate lowerlimit upperlimit)
                        %end;
                        %if &_km=1 %then %do;
                            _km (keep=model _varname_ _varlvl_ timelist _time_ survival sdf_lcl sdf_ucl)
                        %end;;
                        set _surv_adj (rename=(t1=_time_ s1=_surv_ lcl1=_lcl_ ucl1=_ucl_)) end=last;
                        by model _varname_ _varlvl_ _time_;
                        array times {&SYSNOBS} _temporary_;
                        array surv {&SYSNOBS} _temporary_;
                        array lcl {&SYSNOBS} _temporary_;
                        array ucl {&SYSNOBS} _temporary_;
                        if first._varname_ then call missing(of times(*),of surv(*),of lcl(*), of ucl(*));
                        %if &_km=1 %then %do;
                            array _timepoints {%sysfunc(countw(%superq(timelist),%str( )))} (%sysfunc(tranwrd(&timelist,%str( ),%str(,))));
                        %end;
                        ...
                        if last._varlvl_ then do;
                            %if &_median=1 %then %do;
                                /**Median Time-to-Event**/
                                estimate=.;lowerlimit=.;upperlimit=.;percent=50;sdf_lcl=.;sdf_ucl=.;
                                do i = 1 to _count_;
                                    if _order_=1 and ^missing(surv(i)) then do;
                                        if . lt surv(i) le 0.5*_mult_ and estimate=. then estimate=times(i);
                                        if . lt lcl(i) le 0.5*_mult_ and lowerlimit=. then lowerlimit=times(i);
                                        if . lt ucl(i) le 0.5*_mult_ and upperlimit=. then upperlimit=times(i);
                                    end;
                                    else if _order_=2 and ^missing(surv(i)) then do;
                                        if . lt surv(i) ge 0.5*_mult_ and estimate=. then estimate=times(i);
                                        if . lt lcl(i) ge 0.5*_mult_ and lowerlimit=. then lowerlimit=times(i);
                                        if . lt ucl(i) ge 0.5*_mult_ and upperlimit=. then upperlimit=times(i);
                                    end;
                                end;
                                output _quart;
                            %end;
                ...
                %if %sysevalf(%superq(surv_method)=%str(1-KM),boolean) and &_km=1 %then %do;
                    proc sql;
                        update _km
            ...
            %end;
            %else %do;
                proc lifetest data=_tempdsn&i._p2 outcif=_cif /*Outputs estimates for quartiles*/
                    %if &_km=1 %then %do;
                        timelist=&timelist /**Gives list of time-points for event-free rates**/
                    %end;
                        conftype=&conftype /**Method for confidence limits**/;
                ...
                proc sql;
                    create table _quart as
                        select model,_varname_, _varval_, 50 as percent,
                        min(ifn(cif>=0.5,_time_,.)) as estimate,
                        min(ifn(cif_ucl>=0.5,_time_,.)) as lowerlimit,
                        min(ifn(cif_lcl>=0.5,_time_,.)) as upperlimit
                        from _cif
                        group by model,_varname_, _varval_;
                    drop table _cif;
                quit;
            %end;
        %end;
        %if %sysevalf(%qupcase(&method)=SURVIVAL,boolean) and ^(&&_ncat&i>0 or &&_ncont&i>0) and
            %sysevalf(%superq(by&i)^=,boolean) and %sysevalf(%superq(pval_by&i)^=0,boolean) and &_pval=1 %then %do;
            proc sort data=_tempdsn&i._p2 out=_tempdsn&i._p3;
                by modelnum _rowby_lvl _colby_lvl _groupby_lvl _by_num _by_lvl;
                where _varname_='_all_';
            run;
            ...
            %if %sysevalf(%qupcase(&surv_method)^=CIF,boolean) %then %do;
                proc lifetest data=_tempdsn&i._p3;
                    by modelnum _rowby_lvl _colby_lvl _groupby_lvl _by_num;
                    where ^missing(_by_);
                    strata 
        ...
        %if (&&_ncat&i>0 or &&_ncont&i>0) and %sysevalf(%qupcase(&method)=LOGISTIC,boolean) and 
            ...
            %if %sysfunc(exist(_t3))=0 %then %do;
                %if %sysfunc(find(%superq(pval_type3),1)) %then %put WARNING: LOGISTIC procedure requires at least one categorical covariate to produce type-3 p-values;
                proc sql;
                    create table _t3 
                        (model num,effect char(32),probchisq num);
                quit;
            %end;
            %if &_cindex=1 and &&nstrata&i=0 %then %do; 
        ...
        %end;
        /*Cox Models*/
        %if (&&_ncat&i>0 or &&_ncont&i>0) and %sysevalf(%qupcase(&method)=SURVIVAL,boolean) and &_interp=1 %then %do;
            proc phreg data=_tempdsn&i.;
                /**Splits analysis by current BY variable**/
                by modelnum _rowby_lvl _colby_lvl _groupby_lvl _by_num;
                %if &&nstrata&i >0 %then %do;                                   
                    strata %do j=1 %to &&nstrata&i; 
            ...
            run;
        %end;
        %if (&&_ncat&i>0 or &&_ncont&i>0) and %sysevalf(%qupcase(&method)=SURVIVAL,boolean) and 
                (&_hr=1 or &_cindex=1 or
                (&_pval=1 and (&&pval_covariates&i>0 or &&pval_type3&i>0))) %then %do;
            proc phreg data=_tempdsn&i.;
                /**Splits analysis by current BY variable**/
                by model;
                %if &&nstrata&i >0 %then %do;                                   
            ...
            %if &_cindex=1 %then %do;
                ...
                data _stratlevels;
                    ...
                    %if &&nstrata&i>0 %then %do;
                        ...
                        if first._strata_&&nstrata&i then _strata_+1;
                    %end;
        ...
        proc sql;
            %if %qupcase(&method)=SURVIVAL %then %do;
                ...
                %let _classvalcheck=;
                %if %sysfunc(exist(_parm))>0 %then %do;
                ...
                %end;
                create table _tempdsn&i._final as
                        select a.*,. as null
                        %if %sysfunc(exist(_sum))>0 and &_sum=1 %then %do;
                        ...
                        %end;
                        %if %sysfunc(exist(_quart))>0 and &_median=1 %then %do;
                            ...
                            ,ifc(^missing(e.model),
                                ...
                                '') as ref_med_range 
                        %end;
                        %if %sysfunc(exist(_km))>0 and &_km=1 %then %do;
                            %local nkm;
                            ...
                            %end;
                        %end;
                        %if (&&_ncat&i>0 or &&_ncont&i>0) %then %do;
                            %if %sysfunc(exist(_parm))>0 and &_hr=1 %then %do;
                            ...
                            %end;
                            %if %sysfunc(exist(_t3))>0 and &_hr=1 %then %do;
                                ,ifc(^missing(g.model),
                                      case (a.pval_type3)
                            ...
                            %end;
                            %if %sysfunc(exist(_interp))>0 and &_interp=1 %then %do;
                                ,ifc(^missing(ip.modelnum),
                                      case (a.pval_inter)
                                         when 1 then strip(put(ip.probchisq,pvalue6.&pvaldigits))
                                         %if %qupcase(&method)=SURVIVAL and %sysevalf(%qupcase(&surv_method)^=CIF,boolean) %then %do;
                                             when 2 then strip(put(ip.probscorechisq,pvalue6.&pvaldigits))
                                             when 3 then strip(put(ip.problrchisq,pvalue6.&pvaldigits))
                                         %end;
                                      else '' end,'') as inter_pval
                                ,ifc(^missing(ip.modelnum),
                                      case (a.pval_inter)
                                         when 1 then ifc(a.nstrata>0,'S','')||'IPWALD'
                                         when 2 then ifc(a.nstrata>0,'S','')||'IPSCORE'
                                         when 3 then ifc(a.nstrata>0,'S','')||'IPLR'
                                      else '' end,'') as inter_pval_type
                            %end;
                            %if %sysfunc(exist(_fit))>0 and &_fit=1 %then %do;
                            ...
                            %end;  
                            %if %sysfunc(exist(_cindex))>0 and &_cindex=1 %then %do;                            
                                ,m.c as c_est
                            ...
                            %end;  
                        %end;
                        %if %sysfunc(exist(_kmp))>0 and &&pval_by&i>0 and %sysevalf(%superq(by&i)^=,boolean) %then %do;
                            ...
                            left join _sum as b
                                on a.model=b.model and 
                                    ...
                                    ifc(modelsum_row=1 and ^(a.varlevel>0 and a.cov_type=2 and a.show_modelstats=2),'all',case (a.cov_type)
                                        %if &&surv_adj_method&i=0 %then %do;
                                            when 1 then ifc(a.varlevel>0 and a.show_modelstats=2,'all','')
                                        %end;
                        ...
                        %end;
                        %if %sysfunc(exist(_quart))>0 and &_median=1 %then %do;
                            left join (select * from _quart %if &&surv_adj_method&i=2 %then %do; where upcase(_varname_)^='_ALL_' %end;) as d
                            on a.model=d.model and 
                                upcase(ifc(modelsum_Row=1 and ^(a.varlevel>0 and a.cov_type=2 and a.show_modelstats=2),'_all_',case (a.cov_type)
                                   when 1 then ifc(a.varlevel>0 and a.show_modelstats=2,'_all_','')
                                   when 2 then ifc(a.varlevel>0 and (a.display=4 or a.show_modelstats=2),covariate,'')
                                else '' end)) = upcase(d._varname_) and 
                                %if &&surv_adj_method&i=1 %then %do;
                                    varlevel =d._varlvl_
                                %end;
                                %else %do;
                                    ifc(modelsum_row=1 and ^(a.varlevel>0 and a.cov_type=2 and a.show_modelstats=2),'all',case (a.cov_type)
                        ...
                        %if %sysfunc(exist(_km))>0 and &_km=1 %then %do;
                            %do k = 1 %to &nkm;
                                left join (select * from _km where timelist=%scan(&timelist,&k,%str( )) %if &&surv_adj_method&i=2 %then %do; and upcase(_varname_)^='_ALL_' %end;) as b&k
                        ...
                        %if (&&_ncat&i>0 or &&_ncont&i>0) %then %do;
                            ...
                            %if %sysfunc(exist(_t3))>0 and &_hr=1 %then %do;
                                left join _t3 as g
                                    ...
                                    upcase(case (a.cov_type)
                                        when 2 then ifc(a.varlevel=0 or (a.cat_nlvl=2 and a.display=1),'_cat_val_'||scan(covariate,3,'_','m'),'')
                                        when 1 then ifc(a.varlevel=0 or (a.display in(1 2)),covariate,'')
                                    else '' end) = upcase(g.effect)
                            %end;
                            %if %sysfunc(exist(_interp))>0 and &_interp=1 %then %do;
                                left join _interp as ip
                                    on a.modelnum=ip.modelnum and a.rowby_lvl=ip._rowby_lvl and a.colby_lvl=ip._colby_lvl and a.groupby_lvl=ip._groupby_lvl and
                                    ifn(a.by_lvl=0,a.by_num,.)=ip._by_num 
                            %end;
                            %if %sysfunc(exist(_fit))>0 and &_fit=1 %then %do;
            ...
            %if %qupcase(&method)=LOGISTIC %then %do;
                ...
                create table _tempdsn&i._final as
                    select a.*,. as null
                    %if %sysfunc(exist(_bin_))>0 and &_sum=1 %then %do;
                    ...
                    %end;
                    %if %sysfunc(exist(_bin_))>0 and &_bin=1 %then %do;
                        ,ifn(^missing(b._bin_),ifn(upcase("&estfmt.")="PPT",1,100)*b._bin_,.) as bin_est
                        ,ifn(upcase("&conftype")="BIN",ifn(^missing(b.l_bin),ifn(upcase("&estfmt.")="PPT",1,100)*b.l_bin,.),ifn(^missing(b.xl_bin),ifn(upcase("&estfmt.")="PPT",1,100)*b.xl_bin,.)) as bin_lcl
                        ,ifn(upcase("&conftype")="BIN",ifn(^missing(b.u_bin),ifn(upcase("&estfmt.")="PPT",1,100)*b.u_bin,.),ifn(^missing(b.xu_bin),ifn(upcase("&estfmt.")="PPT",1,100)*b.xu_bin,.)) as bin_ucl
                        ,ifn(^missing(c._bin_),ifn(upcase("&estfmt.")="PPT",1,100)*c._bin_,.) as ref_bin_est
                        ,ifn(upcase("&conftype")="BIN",ifn(^missing(c.l_bin),ifn(upcase("&estfmt.")="PPT",1,100)*c.l_bin,.),ifn(^missing(c.xl_bin),ifn(upcase("&estfmt.")="PPT",1,100)*c.xl_bin,.)) as ref_bin_lcl
                        ,ifn(upcase("&conftype")="BIN",ifn(^missing(c.u_bin),ifn(upcase("&estfmt.")="PPT",1,100)*c.u_bin,.),ifn(^missing(c.xu_bin),ifn(upcase("&estfmt.")="PPT",1,100)*c.xu_bin,.)) as ref_bin_ucl
                        ,ifc(^missing(b.model),ifc(^missing(calculated bin_est),strip(put(calculated bin_est,12.&bindigits)),'NE'),'') as bin_estimate
                        ,ifc(^missing(c.model),ifc(^missing(calculated ref_bin_est),strip(put(calculated ref_bin_est,12.&bindigits)),'NE'),'') as ref_bin_estimate
                        ,ifc(^missing(b.model),
                            ifc(^missing(calculated bin_est),strip(put(calculated bin_est,12.&bindigits)),'NE')||' ('||
                            ifc(^missing(b.l_bin),strip(put(calculated bin_lcl,12.&bindigits)),'NE')||'-'||
                            ifc(^missing(b.u_bin),strip(put(calculated bin_ucl,12.&bindigits)),'NE')||')',
                            '') as bin_est_range
                        ,ifc(^missing(c.model),
                            ifc(^missing(calculated ref_bin_est),strip(put(calculated ref_bin_est,12.&bindigits)),'NE')||' ('||
                            ifc(^missing(c.l_bin),strip(put(calculated ref_bin_lcl,12.&bindigits)),'NE')||'-'||
                            ifc(^missing(c.u_bin),strip(put(calculated ref_bin_ucl,12.&bindigits)),'NE')||')',
                    ...
                    %end;
                    %if (&&_ncat&i>0 or &&_ncont&i>0) %then %do;
                        %if %sysfunc(exist(_parm))>0 and %sysfunc(exist(_odds))>0 and &_hr=1 %then %do;
                            ...
                            %end;
                        %end;                    
                        %if %sysfunc(exist(_t3))>0 and &_pval=1 %then %do;                        
                            ,ifc(^missing(h.model),
                                  case (a.pval_type3)
                                     when 1 then strip(put(h.probchisq,pvalue6.&pvaldigits))
                                     %if %qupcase(&method)=SURVIVAL and %sysevalf(%qupcase(&surv_method)^=CIF,boolean) %then %do;
                        ...
                        %end;
                    %end;
                    %if %sysfunc(exist(_kmp))>0 and %sysevalf(%superq(by&i)^=,boolean) and &&pval_by&i>0 and &&_ncov&i=0 %then %do;
                    ...
                    %end;
                    from _tempdsn&i._0 as a 
                    %if %sysfunc(exist(_bin_))>0 and (&_sum=1 or &_bin=1) %then %do;
        ...
        proc datasets nolist nodetails;
            %if &debug=0 %then %do;
                delete %do j = 0 %to &step; _tempdsn_&j %end;
                    _subset _cindex _gpval _km _kmp _models&i _legit_models&i
                    _parm _quart _sum _tempdsn&i 
                    _tempdsn&i._p2 _tempdsn&i._p3 _t3 _fit
                    _bin_calc_ _bin_ _bin_calc_2 _odds
                    _adjwts _adjwts_prep1 adjwts_prep2 _covs _surv_adj;
            %end;
        quit;
    %end;
    ...
    proc datasets nolist nodetails;
        %if &debug=0 %then %do;
            delete %do i = 0 %to &step; _tempdsn_&i %end;
                %do i = 1 %to &nmodels; _models&i _tempdsn&i _tempdsn&i._p2 _tempdsn&i._p3 _tempdsn&i._0 _legit_models&i %end;
                _subset _cindex _gpval _km _kmp _legit_models
                _parm _quart _sum _tempdsn 
                %do i = 1 %to &nmodels; _tempdsn&i %end;
                _tempdsn_p2 _tempdsn_p3 _t3 _fit _interp
                _bin_calc_ _bin_ _bin_calc_2 _odds;
        %end;
    quit;
    ...
    data _tempdsn_final;
        set %do i = 1 %to &nmodels; _tempdsn&i._final %end;;
    run;            
        
    %local npfoot npfoot2 interp_order;
    %let npfoot=0;%let npfoot2=0;
    %if &_pval=1 or &_interp=1 %then %do;
        data _tempdsn_final;
            ...
            if ^missing(by_pval) then do;
                ...
                else if ^missing(inter_pval) then do;
                    pval=vvalue(inter_pval);pfoot=inter_pval_type;
                end;
        ...
        run;
        proc sql noprint;
            select count(distinct ifc(missing(inter_pval),pfoot,'')),
                count(distinct ifc(^missing(inter_pval),pfoot,''))
                into :npfoot separated by '',:npfoot2 separated by '' 
                from _tempdsn_final;
            %if &_pval^=1 %then %let interp_order=1;
            %else %if &_interp^=1 %then %let interp_order=0;
            %else %if 0<%sysfunc(find(%superq(display),inter,i)) and %sysfunc(find(%superq(display),inter,i)) < %sysfunc(find(%sysfunc(tranwrd(%qupcase(%superq(display)),PVAL_INTER,PMAL_INTER)),pval,i)) %then %let interp_order=1;
            %else %let interp_order=0;            
        quit;
        
        data _tempdsn_final;
            set _tempdsn_final end=last;
                length pfoot  _pval pval_listing _inter_pval inter_pval_listing $30.;
                array pfoots {40} $30.;
                array pfs {28} $30. ("GSCORE","GLR","GWALD","T3SCORE","T3LR","T3WALD","COVWALD","LOGRANK","WILCOXON",
                    "SGSCORE","SGLR","SGWALD","ST3SCORE","ST3LR",
                    "ST3WALD","SCOVWALD","SLOGRANK","SWILCOXON","CHISQ","FISHER","GRAY","SGRAY",
                    "IPWALD","IPSCORE","IPLR","SIPWALD","SIPSCORE","SIPLR");
                array pmsgs {28} $100. ("Global score p-value",
                    "Global likelihood-ratio p-value","Global Wald p-value",
                    "Type 3 score p-value","Type 3 likelihood-ratio p-value","Type 3 Wald p-value",
                    "Covariate Wald p-value","Logrank p-value","Wilcoxon p-value","Stratified global score p-value",
                    "Stratified global likelihood-ratio p-value","Stratified global Wald p-value",
                    "Stratified type 3 score p-value","Stratified type 3 likelihood-ratio p-value","Stratified type 3 Wald p-value",
                    "Stratified covariate Wald p-value","Stratified logrank p-value","Stratified Wilcoxon p-value",
                    "Chi-square p-value","Fisher exact p-value",
                    "Gray's Test for Equality","Stratified Gray's Test for Equality",
                    "Type 3 score p-value of interaction term from separate model",
                    "Type 3 likelihood-ratio p-value of interaction term from separate model",
                    "Type 3 Wald p-value of interaction term from separate model",
                    "Stratified type 3 score p-value of interaction term from separate model",
                    "Stratified type 3 likelihood-ratio p-value of interaction term from separate model",
                    "Stratified type 3 Wald p-value of interaction term from separate model");               
                
                %if &npfoot>1 and &npfoot2>1 %then %do;
                    if ^missing(pval) then do i = 1 to dim(pfoots);
                        if missing(pfoots(i)) then do;
                            pfoots(i)=strip(pfoot);
                            pfoot_index=i;
                            i=dim(pfoots);
                            if ^missing(inter_pval) then do;
                                _inter_pval=strip(pval) %if &pfoot=1 %then %do; ||'^{super '||strip(put(pfoot_index,12.0))||'}' %end;;
                                inter_pval_listing=strip(pval) %if &pfoot=1 %then %do; ||repeat('*',pfoot_index-1) %end;;
                                call missing(pval);
                            end;
                            else do;
                ...
                %end;
                %else %if &npfoot>1 %then %do;
                    if ^missing(pval) and missing(inter_pval) then do i = %eval(1+&interp_order) to dim(pfoots);
                        if missing(pfoots(i)) then do;
                            pfoots(i)=strip(pfoot);
                            pfoot_index=i;
                            i=dim(pfoots);
                    ...
                    else if ^missing(pval) and ^missing(inter_pval) then do;
                        %if &interp_order=1 %then %do; pfoot_index=1;%end;
                        %else %do; pfoot_index=&npfoot+1; %end;
                        pfoots(pfoot_index)=strip(pfoot);
                        _inter_pval=strip(pval) %if &pfoot=1 %then %do; ||'^{super '||strip(put(pfoot_index,12.0))||'}' %end;;
                        inter_pval_listing=strip(pval) %if &pfoot=1 %then %do; ||repeat('*',pfoot_index-1) %end;;
                        call missing(pval);
                    end;
                %end;
                %else %if &npfoot2>1 %then %do;
                    if ^missing(pval) and ^missing(inter_pval) then do i = %eval(2-&interp_order) to dim(pfoots);
                        if missing(pfoots(i)) then do;
                            pfoots(i)=strip(pfoot);
                            pfoot_index=i;
                            i=dim(pfoots);
                        ...
                        %if &interp_order=0 %then %do; pfoot_index=1;%end;
                        %else %do; pfoot_index=&npfoot+1; %end;
                        pfoots(pfoot_index)=strip(pfoot);
                        _pval=strip(pval) %if &pfoot=1 %then %do; ||'^{super '||strip(put(pfoot_index,12.0))||'}' %end;;
                        pval_listing=strip(pval) %if &pfoot=1 %then %do; ||repeat('*',pfoot_index-1) %end;;
                ...
                %end;
                %else %do;
                     if ^missing(pval) and missing(inter_pval) then do;
                        %if &interp_order=0 %then %do; pfoot_index=1;%end;
                        %else %do; pfoot_index=2; %end;
                        pfoots(pfoot_index)=strip(pfoot);
                        _pval=strip(pval) %if &pfoot=1 %then %do; ||'^{super '||strip(put(pfoot_index,12.0))||'}' %end;;
                        pval_listing=strip(pval) %if &pfoot=1 %then %do; ||repeat('*',pfoot_index-1) %end;;
                        call missing(pval);
                ...
                retain pfoots;
                drop i j pfoots: pfs: pmsgs: pfoot pval pval_inter;
                rename _pval=pval _inter_pval=pval_inter inter_pval_listing=pval_inter_listing;
        run;
    %end;
    
    %if (&npfoot=1 or &npfoot2=1) and &pfoot=1 %then %do;
        ...
        data _tempdsn_final;
            ...
            %end;
            %if &npfoot2=1 %then %do;
                if ^missing(pval_inter) then pval_inter=scan(pval_inter,1,'^');
                pval_inter_listing=compress(pval_inter_listing,'*');
            %end;
        run;
    %end;
    ...
    proc sql noprint;
        ...
        select max(^missing(colby)),max(^missing(rowby)) into :_colby separated by ' ',:_rowby separated by ' '
            from _tempdsn_final;
        /**Check how many different survival methods**/
        %if %qupcase(&method)=SURVIVAL %then %do;
            %local n_surv_methods;
            select count(distinct catx('-',surv_method,surv_adj_method)) into :n_surv_methods separated by ''
                from _tempdsn_final;
        %end;
    quit;
    %if &show_table or %sysevalf(%superq(out_table)^=,boolean) %then %do;
        data _table;
            merge %do k=1 %to &_columns;
                _tempdsn_final (keep=modelnum--by_lvl  subtitle subind boldind varnum varlevel
                             %do j=1 %to &_ntable_display;
                                 %qupcase(%superq(_table_display&j.))
                                 %if %sysevalf(%qupcase(%superq(_table_display&j.))=PVAL,boolean) %then %do;
                                     %qupcase(%superq(_table_display&j.)_listing)
                                 %end;
                                 %if %sysevalf(%qupcase(%superq(_table_display&j.))=PVAL_INTER,boolean) %then %do;
                                     %qupcase(%superq(_table_display&j.)_listing)
                                 %end;
                             %end;
                         rename=(colby=colby_&k colby_lvl=colbylvl_&k
                             %do j=1 %to &_ntable_display;
                                 %qupcase(%superq(_table_display&j.))=%qupcase(%superq(_table_display&j.)_&k)
                                 %if %sysevalf(%qupcase(%superq(_table_display&j.))=PVAL,boolean) %then %do;
                                     %qupcase(%superq(_table_display&j.)_listing)=%qupcase(%superq(_table_display&j.)_listing_&k)
                                 %end;
                                 %if %sysevalf(%qupcase(%superq(_table_display&j.))=PVAL_INTER,boolean) %then %do;
                                     %qupcase(%superq(_table_display&j.)_listing)=%qupcase(%superq(_table_display&j.)_listing_&k)
                                 %end;
        ...
        data _table;
            ...
            %else %if &shading=3 %then %do;
                if bylevel=0 or varnum<0 then do;
                    _shade=0;
                    shadeind=0;
                end;
                else do;
                    if first.varnum then _shade+1;
                    shadeind=mod(_shade-1,2);
                end;
                drop _shade;
                retain _shade;
            %end;
    ...
    %end;          
    /*Set up Plot*/
    %if &show_plot=1 or %sysevalf(%superq(out_plot)^=,boolean) %then %do;
        data _plot;
            merge %do k=1 %to &_columns;
                _tempdsn_final (in=in&k keep=modelnum--by_lvl subtitle subind boldind varnum varlevel groupby_lvl
                             %let _nplot_=0;
                             %do j=1 %to %sysfunc(countw(%superq(display),%str( )));
                                 %if %sysfunc(find(%scan(%superq(display),&j,%str( )),plot,i))>0 %then %do;
                                    %let _nplot_=%sysevalf(&_nplot_ + 1);
                                    %if %sysfunc(find(%scan(%superq(display),&j,%str( )),KM_PLOT,i))=0 %then %do;
                                        %scan(%scan(%superq(display),&j,%str( )),1,_)_est
                                        %scan(%scan(%superq(display),&j,%str( )),1,_)_lcl
                                        %scan(%scan(%superq(display),&j,%str( )),1,_)_ucl
                                    %end;
                                    %else %do;
                                        %let i=%sysfunc(compress(%scan(%superq(display),&j,%str( )),KM_PLOT,i));
                                        %scan(%scan(%superq(display),&j,%str( )),1,_)_est&i
                                        %scan(%scan(%superq(display),&j,%str( )),1,_)_lcl&i
                                        %scan(%scan(%superq(display),&j,%str( )),1,_)_ucl&i
                                    %end;                                        
                                 %end;
                                 %else %if %sysevalf(%qupcase(%scan(%superq(display),&j,%str( )))=PVAL,boolean) %then %do;
                                    %scan(%superq(display),&j,%str( )) pfoot_index
                                 %end;
                                 %else %if %sysevalf(%qupcase(%scan(%superq(display),&j,%str( )))=PVAL_INTER,boolean) %then %do;
                                    %scan(%superq(display),&j,%str( )) %if &_pval^=1 %then %do; pfoot_index %end;
                                 %end;
                                 %else %if %sysevalf(%qupcase(%scan(%superq(display),&j,%str( )))^=SUBTITLE,boolean) %then %do;
                                    %scan(%superq(display),&j,%str( ))
                                 %end;
                             %end;
                         rename=(colby=colby_&k colby_lvl=colbylvl_&k
                             %let _nplot_=0;
                             %do j=1 %to %sysfunc(countw(%superq(display),%str( )));
                                 %if %sysfunc(find(%scan(%superq(display),&j,%str( )),plot,i))>0 %then %do;
                                    %let _nplot_=%sysevalf(&_nplot_ + 1);
                                    %if %sysfunc(find(%scan(%superq(display),&j,%str( )),KM_PLOT,i))=0 %then %do;
                                        %scan(%scan(%superq(display),&j,%str( )),1,_)_est=estimate&_nplot_._&k 
                                        %scan(%scan(%superq(display),&j,%str( )),1,_)_lcl=lcl&_nplot_._&k 
                                        %scan(%scan(%superq(display),&j,%str( )),1,_)_ucl=ucl&_nplot_._&k
                                    %end;
                                    %else %do;
                                        %let i=%sysfunc(compress(%scan(%superq(display),&j,%str( )),KM_PLOT,i));
                                        %scan(%scan(%superq(display),&j,%str( )),1,_)_est&i=estimate&_nplot_._&k 
                                        %scan(%scan(%superq(display),&j,%str( )),1,_)_lcl&i=lcl&_nplot_._&k 
                                        %scan(%scan(%superq(display),&j,%str( )),1,_)_ucl&i=ucl&_nplot_._&k
                                    %end;
                                 %end;
                                 %else %if %sysevalf(%qupcase(%scan(%superq(display),&j,%str( )))=PVAL,boolean) %then %do;
                                    %scan(%superq(display),&j,%str( ))=%scan(%superq(display),&j,%str( ))_&k 
                                    pfoot_index=pfoot_index_&k
                                 %end;
                                 %else %if %sysevalf(%qupcase(%scan(%superq(display),&j,%str( )))=PVAL_INTER,boolean) %then %do;
                                    %scan(%superq(display),&j,%str( ))=%scan(%superq(display),&j,%str( ))_&k 
                                    %if &_pval^=1 %then %do; pfoot_index=pfoot_index_&k %end;
                                 %end;
                                 %else %if %sysevalf(%qupcase(%scan(%superq(display),&j,%str( )))^=SUBTITLE,boolean) %then %do;
                                    %scan(%superq(display),&j,%str( ))=%scan(%superq(display),&j,%str( ))_&k
                                 %end;
                 ...
                 %end;;  
             by rowby_lvl modelnum by_num by_lvl varnum varlevel groupby_lvl;
             %if &_pval=1 and &pfoot=1 %then %do i=1 %to &_columns;
                pval_&i=scan(pval_&i,1,'^');
             %end;
             %if &_interp=1 and &pfoot=1 %then %do i=1 %to &_columns;
                pval_inter_&i=scan(pval_inter_&i,1,'^');
             %end;
        run;  
        %if %sysevalf(%superq(_colby)=1,boolean) %then %do;
                        ...
                        (select %do j = 1 %to %superq(_nplot_display);
                                    ...
                                    %if %sysfunc(find(%superq(_plot_display&j),plot,i))=0 %then %do;
                                        max(%do i=1 %to %sysfunc(countw(%superq(_plot_displayheader&j),`,m));
                                            ...
                                            %end;
                                            %if %sysevalf(%qupcase(%superq(_plot_display&j))=PVAL,boolean) or %sysevalf(%qupcase(%superq(_plot_display&j))=PVAL_INTER,boolean) %then %do;
        ...
        *options nonotes;
        proc template;
            ...
            begingraph / designheight=&height designwidth=&width    
                ...
                %end;
                %if (&npfoot > 0 or &npfoot2>0) and &pfoot=1 %then %do;
                    entryfootnote halign=left   
                        %do i = 1 %to %sysevalf(&npfoot + &npfoot2);
                            {sup "&i"} "&&pfoot&i; "
                        %end;
                                            ...
                                            %do k = 1 %to %sysfunc(countw(%qupcase(&&_plot_displayheader&j.),`,m));
                                                ...
                                                %if %sysevalf(%superq(_plot_displayheader&j.)^=,boolean) and 
                                                    ...
                                                    drawtext textattrs=(weight=bold  color=&fontcolor family="Albany AMT" size=&sumsize)
                                                        ...
                                                        %if %qupcase(%superq(_plot_display&j.))=PVAL and &npfoot=1 and &pfoot=1 and &k=%sysfunc(countw(%qupcase(&&_plot_displayheader&j.),`,m)) %then %do;
                                                             {sup "%sysevalf(&npfoot2*&interp_order+1)"}
                                                        %end;
                                                        %else %if %qupcase(%superq(_plot_display&j.))=PVAL_INTER and &npfoot2=1 and &pfoot=1  and &k=%sysfunc(countw(%qupcase(&&_plot_displayheader&j.),`,m)) %then %do;
                                                             {sup "%sysevalf(&npfoot*(1-&interp_order)+1)"}
                                                        %end;/ 
                                    ...
                                    %if ^(%qupcase(&&_plot_display&j.) = SUBTITLE and &c>1 and %qupcase(&colsubtitles)=START) and 
                                        ...
                                        layout overlay / opaque=false border=false walldisplay=none;
                                            ...
                                            %else %if %qupcase(&&_plot_display&j.) = PVAL %then %do;   
                                                ...
                                                %end;
                                            %end;                     
                                            %else %if %qupcase(&&_plot_display&j.) = PVAL_INTER %then %do;   
                                                scatterplot x=eval(y*0+1) y=eval(y*0+1) / markerattrs=(size=0pt);                                    
                                                /**Draw Summary Statistics**/
                                                %do k = 1 %to %superq(_maxdv_);
                                                    %if %sysevalf(%qscan(%superq(&&_plot_display&j.._&c),&k,|,m)^=,boolean) and %sysevalf(%qscan(&rowby_lvl,&k,|,m)=&r,boolean) %then %do;
                                                        drawtext 
                                                            textattrs=(weight=normal 
                                                                ...
                                                                %end; family="Albany AMT" size=&sumsize)
                                                            "%scan(%superq(&&_plot_display&j.._&c),&k,|,m)"
                                                                %if %sysevalf(%qscan(%superq(pfoot_index_&c),&k,|,m)^=,boolean) and &pfoot=1 and &npfoot2>1 %then %do;
                                                                    {sup "%qscan(%superq(pfoot_index_&c),&k,|,m)"} 
                                                                %end; / 
                                                            x=50 y=%qscan(%superq(y),&k,|,m)
                                                            xspace=layoutpercent yspace=datavalue anchor=center
        ...
        %if &_listing = 1 %then %do;
            ...
            data _out_listing;
                ...
                %if &_interp=1 %then %do;
                    ...
                    rename  
                        %do c=1 %to &_columns;
                            pval_inter_listing_&c=pval_inter_&c
                        %end;;
                %end;
            run;
            ...
            proc sql noprint;
                ...
                %do j=1 %to  &_ntable_display;
                    ...
                    %if %qupcase(%superq(_table_display&j.))=PVAL and &npfoot=1 and &pfoot=1 and &interp_order=0 %then %let headlength&j=%sysevalf(&&headlength&j+1);
                    %else %if %qupcase(%superq(_table_display&j.))=PVAL and &npfoot=1 and &pfoot=1 and &interp_order=1 %then %let headlength&j=%sysevalf(&&headlength&j+&npfoot2);
                    %else %if %qupcase(%superq(_table_display&j.))=PVAL_INTER and &npfoot2=1 and &pfoot=1 and &interp_order=0 %then %let headlength&j=%sysevalf(&&headlength&j+&npfoot);
                    %else %if %qupcase(%superq(_table_display&j.))=PVAL_INTER and &npfoot2=1 and &pfoot=1 and &interp_order=1 %then %let headlength&j=%sysevalf(&&headlength&j+1);
                    %do i = 1 %to &_columns;
                        %let %qupcase(_list_%superq(_table_display&j.)_&i)=%sysfunc(max(&&headlength&j,%superq(%qupcase(_list_%superq(_table_display&j.)_&i))));
                    %end;
                %end;
                ...
                %do i = 1 %to &_columns;
                    ...
                    %do j=1 %to  &_ntable_display;
                        %let _list_c&i._totlength=%sysevalf(2+&&_list_c&i._totlength+%superq(%qupcase(_list_%superq(_table_display&j.)_&i)));
                    %end;           
            ...
            proc report data=_out_listing nowd split='`' spanrows spacing=0 missing;
                columns
                    ...
                    (rowbylabel rowby_lvl rowby 
                            %if %qupcase(&method)=SURVIVAL  and (&_km=1 or &_median=1)  %then %do;
                                %if &n_surv_methods>1 %then %do;
                                    surv_method surv_adj_method
                                %end;
                            %end;
                            modelnum subind boldind shadeind subtitle groupby)
                        /*Start COLBY Label*/
                        ...
                        %do k = 1 %to &_columns;  
                            ...
                            %do i = 1 %to  &_ntable_display;
                                %qupcase(%superq(_table_display&i.)_&k)
                            %end;    
                    ...
                    %if %qupcase(&method)=SURVIVAL  and (&_km=1 or &_median=1)  %then %do;
                        %if &n_surv_methods>1 %then %do;
                            define surv_method / order order=data noprint;
                            define surv_adj_method / order order=data noprint;
                            compute before surv_adj_method / style={fontweight=bold bordertopstyle=solid borderbottomstyle=solid bordertopwidth=&_htwidth borderbottomwidth=&_htwidth
                                bordertopcolor=black borderbottomcolor=black};
                                if surv_method='CIF' then curr_method='Cumulative Incidence Methods';
                                else if surv_method='KM' then do;
                                    if upcase(surv_adj_method)='UNADJUST' then curr_method='Kaplan-Meier Methods';
                                    else if upcase(surv_adj_method)='DIRECT' then curr_method='Adjusted Kaplan-Meier Methods (Direct Adjustment Method)';
                                    else if upcase(surv_adj_method)='INVWTS' then curr_method='Adjusted Kaplan-Meier Methods (Inverse Weights Method)';
                                end;
                                else if surv_method='1-KM' then do;
                                    if upcase(surv_adj_method)='UNADJUST' then curr_method='1-Survival with Kaplan-Meier Methods';
                                    else if upcase(surv_adj_method)='DIRECT' then curr_method='1-Survival with Adjusted Kaplan-Meier Methods (Direct Adjustment Method)';
                                    else if upcase(surv_adj_method)='INVWTS' then curr_method='1-Survival with Adjusted Kaplan-Meier Methods (Inverse Weights Method)';
                                end;
                                line @1 curr_method $100.;
                            endcomp;
                        %end;
                    %end;
                    ...
                    %do k = 1 %to &_columns;  
                        %do i = 1 %to  &_ntable_display;
                            define %qupcase(%superq(_table_display&i.)_&k) / display center
                                %if (%qupcase(%superq(_table_display&i.))^=PVAL or (&npfoot>1 and &pfoot=1)) and 
                                        (%qupcase(%superq(_table_display&i.))^=PVAL_INTER or (&npfoot2>1 and &pfoot=1)) %then %do;
                                    "%superq(_table_displayheader&i)`%sysfunc(repeat(-,%superq(%qupcase(_list_%superq(_table_display&i.)_&k))-1))"
                                %end;
                                %else %if %qupcase(%superq(_table_display&i.))=PVAL and &npfoot=1 and &pfoot=1 %then %do;
                                    %if &interp_order=1 %then %do;
                                        "%superq(_table_displayheader&i)%sysfunc(repeat(*,&npfoot2))`%sysfunc(repeat(-,%superq(_list_PVAL_&k)-1))"
                                    %end;
                                    %else %do;
                                        "%superq(_table_displayheader&i)*`%sysfunc(repeat(-,%superq(_list_PVAL_&k)-1))"
                                    %end;                                    
                                %end;
                                %else %if %qupcase(%superq(_table_display&i.))=PVAL_INTER and &npfoot2=1 and &pfoot=1 %then %do;
                                    %if &interp_order=0 %then %do;
                                        "%superq(_table_displayheader&i)%sysfunc(repeat(*,&npfoot))`%sysfunc(repeat(-,%superq(_list_PVAL_INTER_&k)-1))"
                                    %end;
                                    %else %do;
                                        "%superq(_table_displayheader&i)*`%sysfunc(repeat(-,%superq(_list_PVAL_INTER_&k)-1))"
                                    %end;                                        
                                %end;;
                        %end;  
                    ...
                    compute after / style={just=l};
                        ...
                        %if (&_interp=1 or &_pval=1) and &pfoot=1 %then %do;
                            line @1 %do i = 1 %to %sysevalf(&npfoot+&npfoot2); 
                                        "%sysfunc(repeat(*,&i-1))%sysfunc(strip(%superq(pfoot&i)));" 
                                    %end;; 
                        %end;  
                        %if %qupcase(&method)=SURVIVAL and (&_km=1 or &_median=1) %then %do;
                            %if &n_surv_methods=1 %then %do;
                                %if %qupcase(&surv_method1)=CIF %then %do;
                                    line @1 'Cumulative Incidence Methods';
                                %end;
                                %else %if %qupcase(&surv_method1)=KM %then %do;
                                    %if &surv_adj_method1=0 %then %do;
                                        line @1 'Kaplan-Meier Methods';
                                    %end;
                                    %else %if &surv_adj_method1=1 %then %do;
                                        line @1 'Adjusted Kaplan-Meier Methods (Direct Adjustment Method)';
                                    %end;
                                    %else %if &surv_adj_method1=2 %then %do;
                                        line @1 'Adjusted Kaplan-Meier Methods (Inverse Weights Method)';
                                    %end;
                                %end;
                                %else %if %qupcase(&surv_method1)=%str(1-KM) %then %do;
                                    %if &surv_adj_method1=0 %then %do;
                                        line @1 '1-Survival with Kaplan-Meier Methods';
                                    %end;
                                    %else %if &surv_adj_method1=1 %then %do;
                                        line @1 '1-Survival with Adjusted Kaplan-Meier Methods (Direct Adjustment Method)';
                                    %end;
                                    %else %if &surv_adj_method1=2 %then %do;
                                        line @1 '1-Survival with Adjusted Kaplan-Meier Methods (Inverse Weights Method)';
                                    %end;
                                %end;
                            %end;
                        %end;
            ...
            %do rloop = 1 %to %sysfunc(countw(&_rloop,|));
                ...
                proc report data=_table nowd split='`' spanrows missing 
                    ...
                    columns
                        ...
                        %end;
                            %do c = 1 %to &_columns;
                               ...
                               /*table_display items*/    
                               %do i = 1 %to &_ntable_display;
                                   %qupcase(%superq(_table_display&i.)_&c)
                               %end;   
                               /*Close COLBY header parenthases*/          
                               %if %sysevalf(%superq(_colby)=1,boolean) %then %do;
                                  )
                    ...
                    %if %sysevalf(%superq(_colby)=1,boolean) %then %do c = 1 %to &_columns;
                        ...
                        %if &c>1 %then %do;
                            define colvald&c / across missing " "
                                style(header)={borderbottomstyle=solid borderbottomwidth=&_htwidth borderbottomcolor=black just=center} ;
                        %end;
                    %end;
                    ...
                    %if %qupcase(&method)=SURVIVAL  and (&_km=1 or &_median=1)  %then %do;
                        %if &n_surv_methods>1 %then %do;
                            ...
                            compute before surv_adj_method / style={fontweight=bold bordertopstyle=solid borderbottomstyle=solid bordertopwidth=&_htwidth borderbottomwidth=&_htwidth
                                ...
                                else if surv_method='1-S' then do;
                                    if upcase(surv_adj_method)='UNADJUST' then curr_method='1-Survival with Kaplan-Meier Methods';
                                    else if upcase(surv_adj_method)='DIRECT' then curr_method='1-Survival with Adjusted Kaplan-Meier Methods (Direct Adjustment Method)';
                                    else if upcase(surv_adj_method)='INVWTS' then curr_method='1-Survival with Adjusted Kaplan-Meier Methods (Inverse Weights Method)';
                                end;
                    ...
                    %do k = 1 %to &_columns;  
                        %do i = 1 %to &_ntable_display;
                            %if %sysfunc(find(%superq(_table_display&i.),KM_,i))>0 %then %do;
                                define %qupcase(%superq(_table_display&i.)_&k) / display center "&_hbtmbrd.%superq(_table_displayheader&i)"
                            ...
                            %else %do;
                                define %qupcase(%superq(_table_display&i.)_&k) / display center
                                    ...
                                    %else %if %qupcase(%superq(_table_display&i.))=PVAL and &npfoot=1 and &pfoot=1 %then %do;
                                        %if &interp_order=1 %then %do;
                                            "&_hbtmbrd.%superq(_table_displayheader&i)^{super %sysevalf(&npfoot2+1)}"
                                        %end;
                                        %else %do;
                                    ...
                                    %else %if %qupcase(%superq(_table_display&i.))=PVAL_INTER and &npfoot2=1 and &pfoot=1 %then %do;
                                        %if &interp_order=0 %then %do;
                                            "&_hbtmbrd.%superq(_table_displayheader&i)^{super %sysevalf(&npfoot+1)}"
                                        %end;
                                        %else %do;
                                            "&_hbtmbrd.%superq(_table_displayheader&i)^{super 1}"
                                        %end;                                        
                    ...
                    compute after / style={leftmargin=0.06in bordertopstyle=solid bordertopwidth=&_htwidth bordertopcolor=black vjust=top 
                       ...
                       %if (&_interp=1 or &_pval=1) and &pfoot=1 %then %do;
                        ...
                        %if %sysevalf(%superq(footnote)=,boolean) =0 %then %do i =1 %to %sysfunc(countw(%superq(footnote),`,m));
                            line @1 "%qscan(%superq(footnote),&i,`,m)";
                        %end;

                        %if ((%sysevalf(%superq(_pval)^=1,boolean) and %sysevalf(%superq(_interp)^=1,boolean)) or %sysevalf(%superq(pfoot)^=1,boolean)) and 
                            %sysevalf(%superq(footnote)=,boolean) %then %do;
                            %if %qupcase(&method)=SURVIVAL %then %do;
                                %if &n_surv_methods>1 or (&_km^=1 and &_median^=1) %then %do;
                                    line @1 ' ';
                                %end;
                            %end;
                            %else %do;
    ...
    %if &debug=0 %then %do; options nonotes; %end;
    /*Output datasets*/
    %if %sysevalf(%superq(out_plot)^=,boolean) %then %do;
        data &out_plot;
            set _plot;
        run;
    %end;
